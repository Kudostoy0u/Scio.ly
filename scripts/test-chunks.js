...
const TEST_CHUNKS = [
  ...
  },
  {
    ...
    patterns: ["src/app/teams/__tests__/calendar-integration.test.*"],
    skip: true, // Skip due to memory issues
  },
  {
    name: "codebusters",
    description: "Codebusters features",
    patterns: ["src/app/codebusters/**/*.test.*"],
  },
  {
    name: "test-features",
    description: "Test and practice features",
    patterns: ["src/app/test/**/*.test.*"],
  },
  {
    name: "unlimited",
    description: "Unlimited practice features",
    patterns: ["src/app/unlimited/**/*.test.*"],
  },
];
...
function runChunk(chunk) {
  ...
  try {
    ...
    const command = `npx vitest run ${patterns} --reporter=verbose --test-timeout=30000`;
