...
const THRESHOLD = parseInt(process.env.LINES || '720', 10);
...
async function listFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (/\.(tsx?|jsx?|md)$/.test(entry.name)) files.push(full);
    }
  }
  return files;
}
...
async function main() {
  ...
  await Promise.all(files.map(async (f) => {
    try {
      ...
      if (n >= THRESHOLD) results.push({ file: path.relative(ROOT, f), lines: n });
    } catch {}
  }));
