...
function processTournament(data, eloData, filePath) {
    ...
    log(`  Input teams: ${data.Teams?.length || 0}, Events: ${data.Events?.length || 0}, Placings: ${data.Placings?.length || 0}`);
...
function analyzeAndRankTeams(data) {
    ...
    log(`    Processing ${data.Placings?.length || 0} placings across ${data.Events?.length || 0} events`);
...
function updateEloForRanking(rankedTeams, eloData, tournamentInfo, category, teamStateMap, filePath) {
    ...
    if (participatingTeams.length < 2) {
        log(`      Skipping ${category}: insufficient participating teams (${participatingTeams.length})`);
        return;
    }
    ...
    const avgInitialRating = Object.values(initialRatings).reduce((sum, r) => sum + r, 0) / Object.keys(initialRatings).length;
    log(`        Initial ratings: avg=${avgInitialRating.toFixed(1)}, range=[${Math.min(...Object.values(initialRatings)).toFixed(1)}, ${Math.max(...Object.values(initialRatings)).toFixed(1)}]`);
    ...
    log(`        Top ${topTeamsCount} teams avg Elo: ${averageElo.toFixed(1)}, competitiveness multiplier: ${competitivenessMultiplier.toFixed(3)}`);
    ...
    log(`        Total Elo change before zero-sum: ${totalEloChange.toFixed(2)}, average: ${averageChange.toFixed(2)}`);
    ...
    log(`        Elo changes after zero-sum: max gain=${maxGain.toFixed(1)}, max loss=${maxLoss.toFixed(1)}`);
    ...
    for (const team of participatingTeams) {
        ...
        if (eloChanges[team.name] < -MAX_ELO_LOSS) {
            log(`        Capping Elo loss for ${team.name}: ${eloChanges[team.name].toFixed(1)} -> -${MAX_ELO_LOSS}`);
        ...
        if (team.name.includes(' Varsity') && initialRatings[team.name] >= 2000 && eloChanges[team.name] <= -90 && !isStateOrNationalTournament(tournamentInfo, filePath)) {
            ...
            log(`        Marking ${team.name} for JV conversion (Elo: ${initialRatings[team.name].toFixed(1)}, change: ${eloChanges[team.name].toFixed(1)})`);
        }
    }
    if (cappedLosses > 0) {
    ...
    if (teamsToConvertToJV.length > 0) {
        log(`        Converting ${teamsToConvertToJV.length} teams from Varsity to JV and recalculating...`);
...
function getTournamentImportanceMultiplier(tournamentInfo, filePath) {
    ...
    try {
        ...
        log(`        Warning: Could not read file contents for importance detection: ${e.message}`);
    }
...
function generateStateFiles(eloData, division, metadata) {
    ...
    const totalTournaments = Object.values(tournamentTimeline).reduce((sum, tournaments) => sum + tournaments.length, 0);
    ...
    log(`  Metadata: ${metadata.teamIds.length} teams, ${metadata.eventIds.length} events, ${metadata.tournamentIds.length} tournaments`);
    ...
    for (const stateCode in eloData) {
        ...
        const fileSize = JSON.stringify(stateData).length;
    ...
    const metaFileSize = JSON.stringify(metadataFile, null, 2).length;
