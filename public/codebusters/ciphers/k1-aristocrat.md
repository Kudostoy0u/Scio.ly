...
Conceptually: index letters against two rows of equal length (26). The top row is the keyed plain alphabet; the bottom row is the standard A–Z. To encrypt, find the plaintext letter in the top row and output the letter directly beneath it in the bottom row. Decryption reverses that.
...
Encryption (K1)
1) Build keyed plain alphabet from the keyword.  
2) For each plaintext letter P, find its index i in the keyed plain alphabet.  
3) Output the i-th letter of the normal cipher alphabet (A–Z).  
4) Leave spaces/punctuation unchanged.

Decryption (K1)
1) For each ciphertext letter C, find its index i in the normal A–Z.  
2) Output the i-th letter of the keyed plain alphabet.  
3) Keep spaces/punctuation unchanged.
...
## Worked Example (Mini)
Keyword: SCIENCE (keyed plain as above)

Encrypt “SEEN”
```
S (top idx 0) → A (bottom idx 0)
E (top idx 3) → D (bottom idx 3)
E (top idx 3) → D (bottom idx 3)
N (top idx 4) → E (bottom idx 4)
Cipher: ADDE
```

Decrypt “JXQLE”
```
J (bottom idx 9)  → top[9]  = G
X (bottom idx 23) → top[23] = X
Q (bottom idx 16) → top[16] = P
L (bottom idx 11) → top[11] = J
E (bottom idx 4)  → top[4]  = N
Plain: GXPJN
```
Check for sense; in real puzzles, you’d adjust with verified mappings.
...
## Practice Exercises
1) With keyword “KEY”, build the K1 top row and encrypt: ATTACKATDAWN.  
2) Given a partially solved mapping, complete the top row and guess the keyword.  
3) For a short cipher, identify THE/AND and reconstruct enough of the top row to read several words.
