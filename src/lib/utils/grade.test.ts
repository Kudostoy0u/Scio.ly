...
import {
	...
	percentageFromEarnedAndTotal,
} from "./grade";

describe("getLetterGradeFromPercentage", () => {
	it("returns N/A for invalid inputs", () => {
		expect(getLetterGradeFromPercentage(Number.NaN as unknown as number)).toBe(
			"N/A",
		);
		expect(getLetterGradeFromPercentage("abc" as unknown as number)).toBe(
			"N/A",
		);
		expect(getLetterGradeFromPercentage(null as unknown as number)).toBe("N/A");
		expect(getLetterGradeFromPercentage(undefined as unknown as number)).toBe(
			"N/A",
		);
	});
...
describe("percentageFromEarnedAndTotal", () => {
	it("handles invalid parameters", () => {
		expect(
			percentageFromEarnedAndTotal(Number.NaN as unknown as number, 10),
		).toBe(0);
		expect(
			percentageFromEarnedAndTotal(5, Number.NaN as unknown as number),
		).toBe(0);
