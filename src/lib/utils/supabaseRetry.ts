...
export async function withAuthRetry<T = unknown>(
  ...
  if (result.error && isAuthError(result.error)) {
    ...
    try {
      ...
      if (refreshError) {
        logger.error(`Session refresh failed for ${operationName}:`, refreshError);
      ...
      if (result.error) {
        logger.error(`${operationName} failed after session refresh:`, result.error);
      }
    } catch (refreshError) {
      logger.error(`Session refresh threw error for ${operationName}:`, refreshError);
      clearSupabaseCookies();
    }
