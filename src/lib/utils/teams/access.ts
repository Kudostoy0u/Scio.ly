...
// Centralized team authorization helpers used across API routes
...
export async function getTeamAccess(
	...
	try {
		...
		const isCreator =
			creatorResult.length > 0 && creatorResult[0]?.createdBy === userId;
		...
		const subteamRole =
			hasSubteamMembership && subteamMemberships[0]
				? subteamMemberships[0].role
				: undefined;
