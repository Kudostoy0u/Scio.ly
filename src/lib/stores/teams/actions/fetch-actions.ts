...
export const createFetchActions = (
	...
	const loadFullTeam = (teamSlug: string) =>
	...
	const fetchUserTeams = async (userId: string): Promise<UserTeam[]> => {
		...
		if (
			isDataFresh(key, CACHE_DURATIONS.userTeams) &&
	...
	const fetchSubteams = async (teamSlug: string): Promise<Subteam[]> => {
		...
		if (
			isDataFresh(key, CACHE_DURATIONS.subteams) &&
	...
	const fetchAssignments = async (teamSlug: string): Promise<Assignment[]> => {
		...
		if (
			isDataFresh(key, CACHE_DURATIONS.assignments) &&
			get().assignments[key]
		) {
	...
	const fetchTournaments = async (
		...
		if (
			isDataFresh(key, CACHE_DURATIONS.tournaments) &&
			get().tournaments[key]
		) {
