import { pgTable, uuid, text, timestamp, boolean, jsonb, bigint, varchar, char } from 'drizzle-orm/pg-core';
...
export const newTeamAssignmentQuestions = pgTable('new_team_assignment_questions', {
  id: uuid('id').primaryKey().defaultRandom(),
  assignmentId: uuid('assignment_id').notNull().references(() => newTeamAssignments.id),
...
export const newTeamAssignmentQuestionResponses = pgTable('new_team_assignment_question_responses', {
  id: uuid('id').primaryKey().defaultRandom(),
  submissionId: uuid('submission_id').notNull().references(() => newTeamAssignmentSubmissions.id),
  questionId: uuid('question_id').notNull().references(() => newTeamAssignmentQuestions.id),
...
export const newTeamAssignmentAnalytics = pgTable('new_team_assignment_analytics', {
  id: uuid('id').primaryKey().defaultRandom(),
  assignmentId: uuid('assignment_id').notNull().references(() => newTeamAssignments.id),
  studentName: text('student_name').notNull(),
  userId: uuid('user_id').references(() => users.id),
  ...
  completionTimeSeconds: bigint('completion_time_seconds', { mode: 'number' }),
...
export const newTeamAssignmentTemplates = pgTable('new_team_assignment_templates', {
  id: uuid('id').primaryKey().defaultRandom(),
  teamId: uuid('team_id').notNull().references(() => newTeamUnits.id),
  createdBy: uuid('created_by').notNull().references(() => users.id),
