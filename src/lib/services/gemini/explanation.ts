...
export class GeminiExplanationService {
  ...
  private async convertImageUrlToBase64(imageUrl: string): Promise<string> {
    try {
      ...
      logger.error("Failed to convert image URL to base64", error as Error, { imageUrl });
      throw new Error(`Failed to convert image URL to base64: ${(error as Error).message}`);
    }
  }
   ...
   */
  private async getImageDataAsBase64(imageData: string | undefined): Promise<string | undefined> {
    ...
    logger.warn("Image data is not a valid URL or base64 data URL", { imageData });
    return undefined;
  }
...
- Focus on helping the user understand the underlying science and improve their knowledge`;
    ...
    return await this.generateStructuredContent(prompt, schema, imageDataBase64);
  }
