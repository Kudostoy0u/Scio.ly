		...
		const postsWithAttachments = posts.map((post) => {
			...
			return {
				...
				post_type: (post.postType || "announcement") as
					...
					| "material"
					| "event",
				priority: (post.priority || "normal") as
					| "low"
					| "normal"
					| "high"
					| "urgent",
		...
		const assignmentsWithSubmissions = await Promise.all(
			assignments.map(async (assignment) => {
				...
				return {
					...
					created_at:
						assignment.createdAt?.toISOString() || new Date().toISOString(),
					updated_at:
						assignment.updatedAt?.toISOString() || new Date().toISOString(),
					creator_email: assignment.creatorEmail,
					creator_name: assignment.creatorName,
					user_submission: submissions[0]
						? {
								status: submissions[0].status as
									| "draft"
									| "submitted"
									| "graded"
									| "returned",
	...
	static async getUserNotifications(
		...
		if (unreadOnly) {
			notificationsQuery = dbPg
				...
				.leftJoin(
					newTeamUnits,
					eq(newTeamNotifications.teamId, newTeamUnits.id),
				)
				.leftJoin(newTeamGroups, eq(newTeamUnits.groupId, newTeamGroups.id))
				.where(
					and(
