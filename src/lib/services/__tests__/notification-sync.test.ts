...
describe("NotificationSyncService", () => {
	...
	describe("syncNotificationToSupabase", () => {
		it("should sync notification successfully", async () => {
			...
			await NotificationSyncService.syncNotificationToSupabase(
				"notification-123",
			);
		...
		it("should throw error if Supabase service key is missing", async () => {
			...
			await expect(
				...
				"Supabase server client not available. SUPABASE_SERVICE_KEY is required.",
			);
		});
	...
	describe("markNotificationAsRead", () => {
		it("should mark notification as read successfully", async () => {
			...
			await NotificationSyncService.markNotificationAsRead(
				"notification-123",
				"user-123",
			);
		...
		it("should throw error if Supabase service key is missing", async () => {
			...
			await expect(
				NotificationSyncService.markNotificationAsRead(
					"notification-123",
					"user-123",
				),
			).rejects.toThrow(
