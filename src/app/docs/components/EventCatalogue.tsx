...
export type EventsByDivision = { B: DocsEvent[]; C: DocsEvent[] };
...
export function EventCatalogue({
	...
	if (
		!(
			eventsByDivision &&
			(eventsByDivision.B?.length > 0 || eventsByDivision.C?.length > 0)
		)
	) {
		return (
	...
	const sorted: EventsByDivision = {
		B: [...(eventsByDivision.B || [])].sort((a, b) =>
			a.name.localeCompare(b.name),
		),
		C: [...(eventsByDivision.C || [])].sort((a, b) =>
			a.name.localeCompare(b.name),
		),
	};
		...
		<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
			{(["B", "C"] as const).map((div) => {
				...
				return (
					<section
						key={div}
						className={`rounded-lg border ${border} ${cardBg} p-5`}
					>
