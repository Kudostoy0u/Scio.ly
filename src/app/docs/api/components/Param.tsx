...
export default function Param({ name, type, required, description }: ParamProps) {
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-md ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
      <div className="flex-1">
        <div className="flex items-center gap-2">
          <code className={`${darkMode ? 'text-gray-100' : 'text-gray-900'} font-mono text-sm font-semibold`}>{name}</code>
          <span className={`${darkMode ? 'text-gray-400' : 'text-gray-500'} text-sm italic`}>{type}</span>
          {required ? (
