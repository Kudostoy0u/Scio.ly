    ...
    <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg border overflow-hidden`}>
      ...
      >
        <div className="flex items-center gap-3">
          <span className={`px-3 py-1 rounded-md text-white text-sm font-semibold ${methodColors[method]}`}>
            {method}
          </span>
          <code className={`${darkMode ? 'text-gray-100' : 'text-gray-900'} font-mono text-sm`}>{url}</code>
          <div className="flex gap-2 ml-auto">
            {features?.map((feature) => (
              <span key={feature} className={`px-2 py-1 text-xs rounded-md ${
