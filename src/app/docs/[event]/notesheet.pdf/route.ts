...
function notesheetPdfDocument({
	evt,
}: { evt: DocsEvent }): React.ReactElement<DocumentProps> {
	...
	return React.createElement(
		...
		React.createElement(
			...
			React.createElement(
				Text,
				{ style: styles.title },
				`Scio.ly Sample Notesheet – ${evt.name} (2026)`,
			),
			React.createElement(
				Text,
				{ style: styles.subtitle },
				`Division: ${evt.division.join(" / ")}`,
			),
			React.createElement(
				View,
				{ style: styles.twoCol },
...
export async function GET(
	...
	if (!evt?.notesheetAllowed) {
		return new NextResponse("Notesheet not available for this event", {
			status: 404,
		});
	}
