...
export function EventSubsectionClient({ evt, sub, md, meta, toc }: EventSubsectionClientProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <EventContent darkMode={darkMode} evt={evt} sub={sub} meta={meta} md={md} />
    </div>
  );
}
    ...
    <aside className="lg:col-span-3 order-last lg:order-first">
      <div className="sticky top-24 space-y-3">
        <div className="flex items-center justify-between">
          <h2 className={`text-sm font-semibold ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
            On this page
          </h2>
          <Link
    ...
    <section>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
          <MetadataRow label="Type" value={meta.typeLabel} darkMode={darkMode} />
          <MetadataRow label="Divisions" value={evt.division.join(", ")} darkMode={darkMode} />
          <MetadataRow label="Participants" value={meta.participants} darkMode={darkMode} />
          <MetadataRow label="Approx. Time" value={meta.approxTime} darkMode={darkMode} />
          <MetadataRow
...
function MetadataRow({
  ...
  return (
    <div className={fullWidth ? "sm:col-span-2" : undefined}>
      ...
      <span className={darkMode ? "text-gray-100" : "text-gray-900"}>{value}</span>
    </div>
  );
}
