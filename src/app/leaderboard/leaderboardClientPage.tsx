...
import { Check, Copy, LogOut, Plus, Trophy, User, Users } from "lucide-react";
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Complex leaderboard page with multiple data sources and interactions
export default function LeaderboardClientPage() {
    ...
    const { error } = await (
      client.from("users") as unknown as {
        update: (values: { display_name: string }) => {
          eq: (column: string, value: string | undefined) => Promise<{ error: unknown }>;
        };
      }
    )
  ...
  const leaveLeaderboard = async (leaderboardId: string) => {
    ...
    )("leave_leaderboard", { p_leaderboard_id: leaderboardId } as Record<string, unknown>);
    if (!error) {
      await loadUserAndLeaderboards();
      setSelectedLeaderboard(null);
    }
            ...
            >
              ...
              <div className="space-y-2">
                ...
                {currentLeaderboards.length === 0 ? (
                  ...
                  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Complex leaderboard rendering logic
                    ...
                    >
                      ...
                      {lb.join_code && (
                        <div className="flex items-center gap-2 mt-2">
                          <code
                            className={`text-xs px-2 py-1 rounded ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
                          >
                            {lb.join_code}
                          </code>
                          <button
            ...
            {selectedLeaderboard && currentLeaderboards.length > 0 ? (
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  <h2 className={`text-2xl font-semibold ${darkMode ? "" : "text-gray-900"}`}>
                    Rankings
                  </h2>
                  <button
                    type="button"
            ...
            {currentLeaderboards.length === 0 && (
              ...
              >
                <p className={darkMode ? "text-gray-300" : "text-gray-700"}>
                  You haven&apos;t joined any leaderboards yet. Use the buttons above to join the
                  public leaderboard, join with a code, or create your own.
                </p>
              </div>
            )}
          </div>
          ...
          >
            ...
            >
              ...
              <p className={`mb-4 text-sm ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
                Please set a display name to join leaderboards. This name will be visible to other
                users.
              </p>
              <input
                type="text"
