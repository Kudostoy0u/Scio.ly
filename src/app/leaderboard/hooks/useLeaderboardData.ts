...
export function useLeaderboardData({
	...
	const [publicLeaderboard, setPublicLeaderboard] =
	...
	const loadUserAndLeaderboards = useCallback(async () => {
      ...
      `)
		...
		if (leaderboardData) {
			...
			const joinedPublic = userLeaderboards.some(
				(lb: Leaderboard) => lb.is_public,
			);
		...
		try {
			const { data: pubLb } = await supabase
				.from("leaderboards")
				.select(
					"id,name,description,is_public,join_code,reset_frequency,created_by",
				)
				.eq("is_public", true)
				.limit(1)
				.single();
