...
import { QuestionPlagiarismSummary, ExtractedQuestion } from '../types';
...
export const QuestionItem = memo(({ 
  ...
  const hasMatches = Boolean(summary && summary.matches && summary.matches.length > 0);

  const getRiskLevel = (questionIndex: number): string | null => {
    const s = questionSummaries.find(ss => ss && ss.questionIndex === questionIndex);
    if (!s || !s.matches || s.matches.length === 0) return null;
    const highest = s.highestSimilarity;
    if (highest >= 0.95) return 'high';
    if (highest >= 0.85) return 'medium';
    if (highest >= 0.40) return 'low';
    return null;
  };
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1 min-w-0">
              <p className="text-slate-800 font-medium text-base leading-relaxed break-words">{question.question}</p>
            </div>
            {(() => {
              if (!summary) {
                return (
              ...
              if (hasMatches) {
                return (
                  <span className="ml-4 flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium bg-green-100 text-green-700 border border-green-200">No Risk</span>
                );
              }

              return (
                <span className="ml-4 flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium bg-green-100 text-green-700 border border-green-200">No Matches</span>
              );
            })()}
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className={`px-3 py-1.5 rounded-full text-xs font-medium border ${
              question.type === 'mcq' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-green-50 text-green-700 border-green-200'
            }`}>
              {question.type === 'mcq' ? 'Multiple Choice' : 'Free Response'}
            </span>
            {summary && hasMatches && (
              <span className="text-xs text-slate-500">{summary.matches.length} match{summary.matches.length !== 1 ? 'es' : ''} found</span>
            )}
          </div>
        </div>
      </div>
