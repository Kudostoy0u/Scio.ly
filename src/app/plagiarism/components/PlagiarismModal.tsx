...
export const PlagiarismModal = memo(
	({ isOpen, onClose, summary }: ModalProps) => {
		if (!(isOpen && summary)) {
			return null;
		}
							...
							{hasMatches ? (
								<div className="space-y-4">
									{matches.map((match: PlagiarismMatch, index: number) => (
										...
										>
											<div
												className={`p-3 ${getSimilarityColor(match.similarity)}`}
											>
											...
											<div className="p-6 bg-white">
												...
												{match.matchedQuestion.options?.length > 0 && (
													<div className="mb-4 space-y-2">
														...
														{match.matchedQuestion.options.map(
															(option: string, i: number) => (
																<div
												...
												<div className="flex flex-wrap gap-2 text-xs">
													...
													<span className="px-3 py-1.5 bg-slate-100 text-slate-700 rounded-full border border-slate-200">
														⭐ Difficulty: {(() => {
															const v = Number(
																match.matchedQuestion.difficulty,
															);
								...
								<div className="text-center py-8 text-slate-500">
									...
									<p className="text-sm mt-1">
										This question appears to be original content.
									</p>
								</div>
							)}
						</div>
