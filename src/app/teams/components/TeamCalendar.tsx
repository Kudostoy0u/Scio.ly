...
export default function TeamCalendar({ teamId: _teamId, isCaptain, teamSlug }: TeamCalendarProps) {
  ...
  const handleDeleteEvent = (eventId: string) => {
    if (window.confirm("Are you sure you want to delete this event?")) {
      deleteEvent(eventId, user?.id, teamSlug, setEvents, setRecurringMeetings, loadEvents).catch(
        () => {
  ...
  const handleCreateRecurringMeeting = async () => {
    await createRecurringMeeting(recurringForm, user, userTeams, teamSlug, () => {
