...
export default function TeamCalendar({ teamId: _teamId, isCaptain, teamSlug }: TeamCalendarProps) {
  ...
  const isEventBlacklisted = useCallback(
  ...
  const filterValidEventsWithBlacklist = useCallback(
  ...
  const filterValidRecurringMeetingsWithBlacklist = useCallback(
  ...
  const createRecurringMeeting = async () => {
    ...
    try {
      ...
      if (recurringForm.meeting_type === "personal") {
        ...
        } else if (userTeams.length > 0 && userTeams[0]) {
      ...
      if (
        !(error instanceof Error && error.message.includes("Failed to create recurring meeting"))
      ) {
        toast.error(
