...
describe("TeamCalendar", () => {
	...
	describe("Event Loading", () => {
		it("loads team events on mount", async () => {
			...
			await waitFor(() => {
				expect(mockFetch).toHaveBeenCalledWith(
					"/api/teams/calendar/events?teamId=test-team-slug",
				);
			});
		});
		...
		it("loads personal events on mount", async () => {
			...
			await waitFor(() => {
				expect(mockFetch).toHaveBeenCalledWith(
					"/api/teams/calendar/personal?userId=test-user-id",
				);
			});
		});
	...
	describe("Form Validation", () => {
		...
		it("creates event without start time (all-day event)", async () => {
			...
			await waitFor(() => {
				expect(
					screen.queryByText("Please enter a start time"),
		...
		it("creates event with only title and date (no times)", async () => {
			...
			const timeInputs = emptyInputs.filter(
				(input) => input.getAttribute("type") === "time",
			);
			...
			await waitFor(() => {
				...
				expect(
					screen.queryByText("End time is required"),
				).not.toBeInTheDocument();
			});
		});
	});
