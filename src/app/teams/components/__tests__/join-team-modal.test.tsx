...
describe("JoinTeamModal", () => {
	...
	it("should handle API errors", async () => {
		const mockOnJoinTeam = vi.fn(() =>
			Promise.reject(new Error("Invalid team code")),
		);

		renderWithProviders(
			<JoinTeamModal {...defaultProps} onJoinTeam={mockOnJoinTeam} />,
		);
	...
	it("should handle dark mode correctly", () => {
		...
		const modal = screen
			.getByText("Join a team")
			.closest('[class*="bg-gray-800"]');
	...
	it("should show help text for team code", () => {
		...
		expect(
			screen.getByText("Ask your team captain for the team code"),
		).toBeInTheDocument();
	});
