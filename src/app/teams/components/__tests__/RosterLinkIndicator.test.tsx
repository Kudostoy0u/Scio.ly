...
describe('RosterLinkIndicator', () => {
  ...
  it('should search users when typing in search input', async () => {
    mockFetch.mockResolvedValueOnce(
      ...
      createMockResponse({ users: [{ id: 'user-123', email: 'john@example.com', display_name: 'John Doe' }] })
    );
  ...
  it('should send invitation when user is selected', async () => {
    mockFetch.mockResolvedValueOnce(
      ...
      createMockResponse({ users: [{ id: 'user-123', email: 'john@example.com', display_name: 'John Doe', username: 'johndoe' }] })
    ).mockResolvedValueOnce(
      createMockResponse({ invitation: { id: 'invitation-123' }, message: 'Invitation sent successfully' })
    );
    ...
    await waitFor(() => {
      ...
      const inviteCalls = mockFetch.mock.calls.filter(call => 
