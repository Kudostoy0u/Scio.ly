...
describe("AssignmentViewer", () => {
	...
	it("should render assignment viewer", () => {
		...
		expect(
			screen.getByText(QUESTION_1_MULTIPLE_CHOICE_REGEX),
		).toBeInTheDocument();
	...
	it("should allow navigation between questions", () => {
		...
		expect(
			screen.getByText(QUESTION_2_FREE_RESPONSE_REGEX),
		).toBeInTheDocument();
	...
	it("should handle free response questions", () => {
		...
		fireEvent.change(textarea, {
			target: { value: "Photosynthesis is the process..." },
		});
	...
	it("should submit assignment when submit button is clicked", async () => {
		...
		mockFetch.mockResolvedValueOnce(
			createMockResponse({ submission: mockSubmission }),
		);
	...
	it("should show error when submission fails", async () => {
		mockFetch.mockResolvedValueOnce(
			createMockResponse({ error: "Submission failed" }, 500),
		);
	...
	it("should prevent submission without answers", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Please answer at least one question"),
			).toBeInTheDocument();
		});
	});
	...
	it("should handle codebusters questions", () => {
		...
		render(
			<AssignmentViewer {...defaultProps} assignment={codebustersAssignment} />,
		);
