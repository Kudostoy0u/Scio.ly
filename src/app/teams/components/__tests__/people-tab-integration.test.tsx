...
const mockTeamStore = {
	...
	getCacheKey: vi.fn(
		(type: string, ...params: string[]) => `${type}-${params.join("-")}`,
	),
};
	...
	const handleRemoveEvent = async (member: Member, eventData: EventData) => {
		try {
			...
			if (response.ok) {
				...
				mockToast.success(
					`Removed ${member.name} from ${eventData.event} in ${eventData.subteam}`,
				);
			}
		} catch (_error) {
			// Ignore errors
				...
				<div key={member.id} data-testid={`member-${member.id}`}>
					...
					<div data-testid={`member-${member.id}-events`}>
						{member.events.map((event, _index) => (
							<div
								key={`${member.id}-${event}`}
								data-testid={`event-${member.id}-${event}`}
							>
								<span>{event}</span>
								<button
									type="button"
...
describe("PeopleTab Integration Tests", () => {
	...
	describe("Event Removal", () => {
		it("should call removeRosterEntry with correct parameters", async () => {
			...
			(
				global.fetch as vi.MockedFunction<typeof global.fetch>
			...
			const removeButton = screen.getByTestId(
				"remove-event-user-123-Astronomy",
			);
			...
			await waitFor(() => {
				expect(global.fetch).toHaveBeenCalledWith(
					"/api/teams/test-team/roster/remove",
					{
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
			...
			expect(mockToast.success).toHaveBeenCalledWith(
				"Removed John Doe from Astronomy in Team A",
			);
		});
		...
		it('should normalize event names with "and" when removing', async () => {
			...
			const TestComponentWithAndEvent = () => {
				...
				const handleRemoveEvent = async (
					...
					if (response.ok) {
						if (eventData.subteamId) {
							const normalizedEventName = eventData.event.replace(
								/\band\b/g,
								"&",
							);
							mockTeamStore.removeRosterEntry(
			...
			const removeButton = screen.getByTestId(
				"remove-event-user-123-Anatomy and Physiology",
			);
			fireEvent.click(removeButton);
						...
						{members.map((member) => (
							<div key={member.id}>
								<button
									type="button"
									onClick={() =>
										handleAddEvent(member, "Anatomy and Physiology")
									}
									data-testid={`add-event-${member.id}`}
								>
