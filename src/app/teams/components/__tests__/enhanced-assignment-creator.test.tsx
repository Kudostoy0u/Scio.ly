...
const getEventSubtopics = vi.hoisted(() =>
	vi.fn(() => ["Subtopic 1", "Subtopic 2"]),
);
...
describe("EnhancedAssignmentCreator", () => {
	...
	describe("Step Navigation", () => {
		it("starts at step 1", () => {
			...
			expect(screen.getByTestId("assignment-details-step")).toBeInTheDocument();
			expect(
				screen.queryByTestId("question-generation-step"),
		...
		it("navigates to step 2 when next is clicked", () => {
			...
			expect(
				screen.getByTestId("question-generation-step"),
			).toBeInTheDocument();
			expect(
				screen.queryByTestId("assignment-details-step"),
			).not.toBeInTheDocument();
		});
	...
	describe("Data Management", () => {
		it("loads roster members on mount", async () => {
			...
			await waitFor(() => {
				expect(fetchRosterMembers).toHaveBeenCalledWith(
					"team123",
					"subteam456",
				);
			});
		...
		it("shows success toast when questions are generated", async () => {
			...
			await waitFor(() => {
				expect(toast.success).toHaveBeenCalledWith(
					"Generated 1 questions successfully!",
				);
			});
		});
