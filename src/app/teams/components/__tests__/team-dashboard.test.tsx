...
const mockCreateSubteamMutation = vi.fn(() =>
	Promise.resolve({ id: "subteam-1" }),
);
...
describe("TeamDashboard", () => {
			...
			() => {
				expect(
					screen.getByText((content, element) => {
						return (
							element?.textContent === "Division B" ||
							content.includes("Division B")
						);
					}),
				).toBeInTheDocument();
			},
	...
	it("should render with stream tab when specified", async () => {
		renderWithProviders(
			<TeamDashboard {...defaultProps} activeTab="stream" />,
			{
				initialUser: mockUser,
			},
		);
	...
	it("should render with assignments tab when specified", async () => {
		renderWithProviders(
			<TeamDashboard {...defaultProps} activeTab="assignments" />,
			{
				initialUser: mockUser,
			},
		);
	...
	it("should render with people tab when specified", async () => {
		renderWithProviders(
			<TeamDashboard {...defaultProps} activeTab="people" />,
			{
				initialUser: mockUser,
			},
		);
	...
	it("should open exit modal when exit button is clicked", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Are you sure you want to exit this team?"),
			).toBeInTheDocument();
		});
	});
