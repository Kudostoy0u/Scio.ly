...
import DivisionGroupsGrid, { DIVISION_B_GROUPS, DIVISION_C_GROUPS, getGroupColors } from '../DivisionGroupsGrid';
...
describe('DivisionGroupsGrid', () => {
  ...
  it('should call setName when input value changes', () => {
    ...
    expect(defaultProps.setName).toHaveBeenCalledWith('B', 'Codebusters', 0, 'Alice Updated');
  });
  ...
  it('should handle multiple teams correctly', () => {
    ...
    render(<DivisionGroupsGrid {...defaultProps} teams={multipleTeams} activeTeamIdx={1} />);
...
describe('DIVISION_B_GROUPS', () => {
  ...
  it('should have correct events for each group', () => {
    expect(DIVISION_B_GROUPS[0].events).toEqual(['Codebusters', 'Disease Detectives', 'Remote Sensing']);
    expect(DIVISION_B_GROUPS[1].events).toEqual(['Entomology', 'Experimental Design', 'Solar System']);
    expect(DIVISION_B_GROUPS[6].events).toEqual(['Boomilever', 'Helicopter', 'Hovercraft', 'Mission Possible', 'Scrambler']);
  });
...
describe('DIVISION_C_GROUPS', () => {
  ...
  it('should have correct events for each group', () => {
    expect(DIVISION_C_GROUPS[0].events).toEqual(['Anatomy & Physiology', 'Engineering CAD', 'Forensics']);
    expect(DIVISION_C_GROUPS[1].events).toEqual(['Codebusters', 'Disease Detectives', 'Remote Sensing']);
    expect(DIVISION_C_GROUPS[6].events).toEqual(['Boomilever', 'Bungee Drop', 'Electric Vehicle', 'Helicopter', 'Hovercraft', 'Robot Tour']);
  });
