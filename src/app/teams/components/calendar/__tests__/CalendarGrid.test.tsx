...
describe("CalendarGrid", () => {
	...
	describe("Event Interactions", () => {
		...
		it("calls onDeleteEvent when delete button is clicked", () => {
			...
			const deleteButton = testEvent
				.closest("button")
				?.querySelector('[title="Delete event"]');
		...
		it("calls onAddEventForDate when add event button is clicked", () => {
			...
			const addButton = dayCell
				?.querySelector('[data-testid="plus-icon"]')
				?.closest("button");
	...
	describe("Empty State", () => {
		it("renders empty calendar when no events or meetings", () => {
			render(
				<CalendarGrid {...defaultProps} events={[]} recurringMeetings={[]} />,
			);
