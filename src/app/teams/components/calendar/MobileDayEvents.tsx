...
function getEventColors(type: string, darkMode: boolean): string {
  switch (type) {
    case 'tournament':
      return darkMode ? 'bg-red-900/20 text-red-300 border-red-800' : 'bg-red-100 text-red-800 border-red-200';
    case 'practice':
      return darkMode ? 'bg-green-900/20 text-green-300 border-green-800' : 'bg-green-100 text-green-800 border-green-200';
    case 'meeting':
      return darkMode ? 'bg-blue-900/20 text-blue-300 border-blue-800' : 'bg-blue-100 text-blue-800 border-blue-200';
    case 'personal':
      return darkMode ? 'bg-green-900/20 text-green-300 border-green-800' : 'bg-green-100 text-green-800 border-green-200';
    case 'deadline':
      return darkMode ? 'bg-orange-900/20 text-orange-300 border-orange-800' : 'bg-orange-100 text-orange-800 border-orange-200';
    default:
      return darkMode ? 'bg-gray-700/50 text-gray-200 border-gray-600' : 'bg-gray-100 text-gray-800 border-gray-200';
  }
}
...
export default function MobileDayEvents({
  ...
  const sameDayEvents = React.useMemo(() => {
    const base = events.filter(e => new Date(e.start_time).toISOString().split('T')[0] === dateStr);
    const dayOfWeek = date.getDay();
    const recurringToday: CalendarEvent[] = recurringMeetings
      .filter(m => {
        if (!Array.isArray(m.days_of_week) || !m.days_of_week.includes(dayOfWeek)) return false;
        if (m.exceptions && Array.isArray(m.exceptions) && m.exceptions.includes(dateStr)) return false;
        if (m.start_date && dateStr < m.start_date) return false;
        if (m.end_date && dateStr > m.end_date) return false;
        const eventId = `recurring-${m.id}-${dateStr}`;
        if (isEventBlacklisted && isEventBlacklisted(eventId)) return false;
        return true;
      })
      .map(m => ({
        ...
        start_time: m.start_time ? `${dateStr}T${m.start_time}` : `${dateStr}T00:00:00`,
    ...
    return [...base, ...recurringToday].sort((a, b) => new Date(a.start_time).getTime() - new Date(b.start_time).getTime());
  }, [events, recurringMeetings, dateStr, date, isEventBlacklisted]);

  if (sameDayEvents.length === 0) {
    return (
      <div className={`rounded-lg border ${darkMode ? 'border-gray-700' : 'border-gray-200'} p-4 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
