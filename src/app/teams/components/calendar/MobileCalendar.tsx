...
export default function MobileCalendar({
  ...
  return (
    <div className={`rounded-lg border ${darkMode ? 'border-gray-700' : 'border-gray-200'} overflow-hidden md:hidden`}>
      {/* Day headers */}
      <div className={`${darkMode ? 'bg-gray-800' : 'bg-gray-50'} grid grid-cols-7`}>
        {dayNamesShort.map((d, i) => (
          <div key={i} className={`p-2 text-center text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{d}</div>
        ))}
      </div>
      {/* Grid */}
      <div className="grid grid-cols-7 grid-rows-6">
        {days.map((date, idx) => {
          ...
          const recurringToday = recurringMeetings.filter((m) => {
            if (!Array.isArray(m.days_of_week) || !m.days_of_week.includes(dayOfWeek)) return false;
            const ds = dateStr;
            if (m.exceptions && Array.isArray(m.exceptions) && m.exceptions.includes(ds)) return false;
            if (m.start_date && ds < m.start_date) return false;
            if (m.end_date && ds > m.end_date) return false;
          ...
          const dotTypes = [...baseEvents.map(e => e.event_type), ...recurringToday.map(e => e.event_type)].slice(0, 4);
          ...
          return (
            <button
              ...
              className={`h-16 p-1 border-r border-b text-left ${
                ...
                selected ? (darkMode ? 'ring-1 ring-blue-500' : 'ring-1 ring-blue-500') : ''
              }`}
            >
              <div className="flex items-center justify-between">
                <span className={`text-xs font-medium ${
                  selected ? 'text-blue-500' : isCurrentMonth(date) ? (darkMode ? 'text-white' : 'text-gray-900') : (darkMode ? 'text-gray-500' : 'text-gray-400')
                }`}>
                  {date.getDate()}
                </span>
              </div>
              <div className="mt-1 flex flex-wrap gap-1">
                {dotTypes.map((t, i) => (
                  <span key={i} className={`w-1.5 h-1.5 rounded-full ${getEventColor(t, darkMode)}`}></span>
                ))}
              </div>
            </button>
          );
