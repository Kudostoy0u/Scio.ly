      ...
      <div className="grid grid-cols-7 grid-rows-6">
        {days.map((date, idx) => {
          ...
          const recurringToday = recurringMeetings
            .filter((m) => {
              if (!(Array.isArray(m.days_of_week) && m.days_of_week.includes(dayOfWeek))) {
                return false;
              }
              const ds = dateStr;
              if (m.exceptions && Array.isArray(m.exceptions) && m.exceptions.includes(ds)) {
                return false;
              }
          ...
          const dotTypes = [
            ...
            ...recurringToday.map((e: { event_type: "meeting" }) => e.event_type),
          ].slice(0, 4);
