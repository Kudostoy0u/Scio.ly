  ...
  try {
    ...
    return Array.from(byDate.entries())
      ...
      .map(([date, list]) => ({
        date,
        tournament:
          list.length <= 2 ? list.join(", ") : `${list[0]}, ${list[1]} and ${list.length - 2} more`,
        allTournaments: list,
        season,
      }));
  } catch {
...
export function formatRankingChange(
  ...
  if (change > 0) {
    return { text: `+${change}`, colorClass: darkMode ? "text-green-400" : "text-green-600" };
  }
  if (change < 0) {
    return { text: `${change}`, colorClass: darkMode ? "text-red-400" : "text-red-600" };
  }
...
export function rankColor(darkMode: boolean, rank: number): string {
  ...
  if (rank === 3) {
    return darkMode ? "bg-orange-900/30 text-orange-200" : "bg-orange-100 text-orange-800";
  }
