...
export default function AssignmentsTab({
	...
	const isUserAssignedToAssignment = (assignment: Assignment): boolean => {
		...
		return assignment.roster.some(
			(rosterMember) => rosterMember.user_id === user.id,
		);
	};
							...
							>
								<div className="flex items-start justify-between">
									<div className="flex-1">
														...
														{showDeleteConfirm === assignmentTyped.id ? (
															<div className="flex items-center space-x-1">
																<button
																	type="button"
																	onClick={() =>
																		handleDeleteAssignment(assignmentTyped.id)
																	}
																	disabled={
															...
															<button
																type="button"
																onClick={() =>
																	setShowDeleteConfirm(assignmentTyped.id)
																}
										...
										{!everyoneDeclined && (
											<>
												{assignmentTyped.roster &&
													assignmentTyped.roster.length > 0 && (
														<div
												...
												>
													{assignmentTyped.due_date && (
														<div className="flex items-center space-x-1">
															...
															<span>
																Due: {formatDate(assignmentTyped.due_date)}
															</span>
														</div>
													)}
													{assignmentTyped.time_limit_minutes && (
														<div className="flex items-center space-x-1">
															...
															<span>
																{assignmentTyped.time_limit_minutes} min
															</span>
														</div>
													)}
													{assignmentTyped.questions_count && (
														<div className="flex items-center space-x-1">
															<span>
																{assignmentTyped.questions_count} questions
															</span>
														</div>
													)}
												...
												{assignmentTyped.user_submission && (
													...
													>
														Submitted:{" "}
														{formatDate(
															assignmentTyped.user_submission.submitted_at,
														)}
													</div>
												)}
											</>
										...
										{!everyoneDeclined &&
											(!isCaptain ||
												(isCaptain &&
													isUserAssignedToAssignment(assignmentTyped))) && (
