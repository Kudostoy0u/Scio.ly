...
import type { CalendarEvent, RecurringMeeting, UserTeam } from "../../calendar/calendarUtils";
import {
...
function loadCachedData(
  ...
  if (cachedRecurring) {
    const validRecurring = filterValidRecurringMeetingsWithBlacklist(cachedRecurring, userId);
...
async function loadTeamEvents(
  ...
  if (eventsRes.ok) {
    ...
    const freshEvents = filterValidEventsWithBlacklist(eventsData.events || [], userId);
...
async function loadPersonalEvents(
  ...
  const personalRes = await fetch(`/api/teams/calendar/personal?userId=${userId}`);
  if (personalRes.ok) {
    const personalData = await personalRes.json();
    const personalEvents = personalData.events || [];
    setEvents((prev) => {
      const teamEvents = teamSlug ? prev : [];
      const combined = filterValidEventsWithBlacklist([...teamEvents, ...personalEvents], userId);
...
async function loadUserTeams(userId: string, setUserTeams: (teams: UserTeam[]) => void) {
...
export function useCalendarData(userId: string | undefined, teamSlug: string | undefined) {
  ...
  const loadEvents = useCallback(async () => {
    ...
    const cacheKey = teamSlug ? `calendar_${teamSlug}` : `calendar_user_${userId}`;
    ...
    try {
      ...
      if (teamSlug) {
        await loadRecurringMeetings(teamSlug, cacheKey, userId, setRecurringMeetings);
      }
