...
function getTargetTeamIds(
	...
	if (eventForm.meeting_type === "team" && eventForm.selected_team_id) {
		if (eventForm.selected_team_id.startsWith("all-")) {
			...
			const schoolTeams = userTeams.filter(
				(team) => team.school === schoolName,
			);
...
function createEventData(
	...
	return {
		...
		created_by: userId,
		team_id: teamId,
		start_time: createTimezoneAwareDateTime(
			eventForm.date,
...
export async function createEvent(
	...
	try {
		...
		const eventPromises = createEventPromises(
			targetTeamIds,
			eventForm,
			user.id,
		);
		...
		if (
			!(
				...
				error.message.includes("Failed to create event")
			)
		) {
...
function validateRecurringForm(recurringForm: RecurringForm): void {
	...
	if (
		...
		!recurringForm.selected_team_id
	) {
...
async function handleRecurringMeetingCreationError(
	responses: Response[],
): Promise<never> {
	const failedResponse = responses.find((res) => !res.ok);
	...
	if (errorMessage.includes("ambiguous")) {
		...
		toast.error(
			"You are not authorized to create recurring meetings for this team",
		);
	} else if (errorMessage.includes("Not a team member")) {
		toast.error(
			"You must be a member of this team to create recurring meetings",
		);
...
export async function createRecurringMeeting(
	...
	try {
		const targetTeamSlugs = getTargetTeamSlugs(
			recurringForm,
			teamSlug,
			userTeams,
		);
		...
		if (
			!(
				error instanceof Error &&
				error.message.includes("Failed to create recurring meeting")
			)
		) {
			toast.error(
...
function filterAndUpdateCache(
	...
	setRecurringMeetings: React.Dispatch<
		React.SetStateAction<RecurringMeeting[]>
	>,
): void {
	const cacheKey = getCacheKey(teamSlug, userId);
	const currentEvents =
		globalApiCache.get<CalendarEvent[]>(`${cacheKey}_events`) || [];
	const currentRecurring =
		globalApiCache.get<RecurringMeeting[]>(`${cacheKey}_recurring`) || [];
