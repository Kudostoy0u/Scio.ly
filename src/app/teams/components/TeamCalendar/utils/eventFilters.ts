...
import type { CalendarEvent, RecurringMeeting } from "../../calendar/calendarUtils";
...
export function filterValidRecurringMeetings(meetings: RecurringMeeting[]): RecurringMeeting[] {
...
export function isEventBlacklisted(eventId: string, userId: string | undefined): boolean {
...
export function blacklistEventId(eventId: string, userId: string | undefined): void {
  const blacklistKey = `blacklisted_events_${userId}`;
  const currentBlacklist = globalApiCache.get<string[]>(blacklistKey) || [];
...
export function filterValidEventsWithBlacklist(
  ...
  return filterValidEvents(events).filter((event) => !isEventBlacklisted(event.id, userId));
}
