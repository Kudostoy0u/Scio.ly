...
describe('assignmentUtils', () => {
  ...
  describe('getEventSubtopics', () => {
    ...
    it('should return subtopics for mapped event (Water Quality)', async () => {
      ...
      (fetch as any).mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, data: ['Freshwater Subtopic 1', 'Freshwater Subtopic 2'] })
      });

      const subtopics = await getEventSubtopics('Water Quality');
      expect(subtopics).toEqual(['Freshwater Subtopic 1', 'Freshwater Subtopic 2']);
      
      // Verify it called the API with the mapped event name
      expect(fetch).toHaveBeenCalledWith('/api/meta/subtopics?event=Water%20Quality%20-%20Freshwater');
    });
    ...
    it('should return subtopics for mapped event (Dynamic Planet)', async () => {
      ...
      (fetch as any).mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, data: ['Oceanography Subtopic 1', 'Oceanography Subtopic 2'] })
      });

      const subtopics = await getEventSubtopics('Dynamic Planet');
      expect(subtopics).toEqual(['Oceanography Subtopic 1', 'Oceanography Subtopic 2']);
      
      // Verify it called the API with the mapped event name
      expect(fetch).toHaveBeenCalledWith('/api/meta/subtopics?event=Dynamic%20Planet%20-%20Oceanography');
    });
    ...
    it('should return combined subtopics for Anatomy & Physiology', async () => {
      ...
      (fetch as any)
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ success: true, data: ['Endocrine Subtopic 1', 'Endocrine Subtopic 2'] })
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ success: true, data: ['Nervous Subtopic 1', 'Nervous Subtopic 2'] })
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ success: true, data: ['Sense Organs Subtopic 1', 'Sense Organs Subtopic 2'] })
        });

      const subtopics = await getEventSubtopics('Anatomy & Physiology');
      expect(subtopics).toEqual([
        'Endocrine Subtopic 1', 'Endocrine Subtopic 2',
        'Nervous Subtopic 1', 'Nervous Subtopic 2',
        'Sense Organs Subtopic 1', 'Sense Organs Subtopic 2'
      ]);
    });
  ...
  describe('generateQuestions', () => {
    it('should generate questions successfully', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('/api/teams/team123/assignments/generate-questions', {
        method: 'POST',
    ...
    it('should throw error when API call fails', async () => {
      ...
      await expect(generateQuestions('Test Event 1', 5, 'both', [], 0, false, 'team123'))
        .rejects.toThrow('Failed to generate questions');
    });
  });

  describe('fetchRosterMembers', () => {
    it('should fetch roster members for team', async () => {
      const mockRosterData = {
        'Event 1': ['John Doe', 'Jane Smith'],
        'Event 2': ['John Doe', 'Bob Wilson']
      };

      const mockMembersData = {
        members: [
          { id: 'user1', name: 'John Doe', email: 'john@example.com', username: 'johndoe' },
          { id: 'user2', name: 'Jane Smith', email: 'jane@example.com', username: 'janesmith' }
        ]
      };
      ...
      const roster = await fetchRosterMembers('team123');

      expect(roster).toEqual([
        { student_name: 'John Doe', isLinked: true, user_id: 'user1', subteam_id: undefined, userEmail: 'john@example.com', username: 'johndoe' },
        { student_name: 'Jane Smith', isLinked: true, user_id: 'user2', subteam_id: undefined, userEmail: 'jane@example.com', username: 'janesmith' },
        { student_name: 'Bob Wilson', isLinked: false, user_id: undefined, subteam_id: undefined, userEmail: undefined, username: undefined }
      ]);
      expect(fetch).toHaveBeenCalledWith('/api/teams/team123/roster');
      expect(fetch).toHaveBeenCalledWith('/api/teams/team123/members');
    });

    it('should fetch roster members for subteam', async () => {
      const mockRosterData = {
        'Event 1': ['John Doe']
      };
      ...
      const roster = await fetchRosterMembers('team123', 'subteam456');

      expect(roster).toEqual([
        { student_name: 'John Doe', isLinked: true, user_id: 'user1', subteam_id: 'subteam456', userEmail: 'john@example.com', username: 'johndoe' }
      ]);
      expect(fetch).toHaveBeenCalledWith('/api/teams/team123/roster?subteamId=subteam456');
      expect(fetch).toHaveBeenCalledWith('/api/teams/team123/members?subteamId=subteam456');
    });
    ...
    it('should throw error when roster API call fails', async () => {
      ...
      await expect(fetchRosterMembers('team123'))
        .rejects.toThrow('Failed to fetch roster data');
    });
    ...
    it('should throw error when members API call fails', async () => {
      ...
      await expect(fetchRosterMembers('team123'))
        .rejects.toThrow('Failed to fetch team members');
    });
  });
  ...
  describe('createAssignment', () => {
    it('should create assignment for team', async () => {
      ...
      const result = await createAssignment('team123', undefined, assignmentData);
    ...
    it('should create assignment for subteam', async () => {
      ...
      const result = await createAssignment('team123', 'subteam456', assignmentData);

      expect(result).toEqual(mockAssignment);
      expect(fetch).toHaveBeenCalledWith('/api/teams/team123/subteams/subteam456/assignments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(assignmentData)
      });
    ...
    it('should throw error when API call fails', async () => {
      ...
      await expect(createAssignment('team123', undefined, assignmentData))
