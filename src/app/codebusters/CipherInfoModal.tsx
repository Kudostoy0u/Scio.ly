...
function createMarkdownComponents(darkMode: boolean) {
  ...
  return {
    h2: (props: { children?: React.ReactNode }) => {
      ...
      return (
        <h2 id={id} className="scroll-mt-20 text-lg sm:text-xl mt-6 mb-2" {...props}>
          {children}
        </h2>
      );
    },
...
const CipherInfoModal: React.FC<CipherInfoModalProps> = ({
  ...
  const fetchMarkdown = useCallback(async (url: string, signal: AbortSignal) => {
  ...
  useScrollSectionTracking(contentRef, sections, activeTabId, setActiveTabId, contentMode);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <ModalHeader cipherType={cipherType} darkMode={darkMode} onClose={onClose} />
          <ContentModeToggle
