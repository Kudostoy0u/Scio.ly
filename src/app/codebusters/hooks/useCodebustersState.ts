...
export const useCodebustersState = (assignmentId?: string | null) => {
  ...
  const handleTestParamsLoading = useCallback(
    (
      ...
      if (savedQuotes) {
        loadSavedQuotesWithTestParams(savedQuotes, savedIsTestSubmitted, savedTestScore);
      } else {
  ...
  const initializeTestSessionFromStorage = useCallback(
    (testParamsStr: string, hasShareCode: string | null, savedIsTestSubmitted: string | null) => {
      ...
      const session = getOrCreateTestSession(eventName, timeLimit, hasShareCode);
  ...
  useEffect(() => {
    ...
    if (
      savedQuotes &&
      loadQuotesFromSavedState(savedQuotes, savedIsTestSubmitted, savedTestScore)
    ) {
      return;
    }
    ...
    if (testParamsStr) {
      ...
      initializeTestSessionFromStorage(testParamsStr, hasShareCode, savedIsTestSubmitted);
    } else {
