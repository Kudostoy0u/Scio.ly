...
import { revealCheckerboardLetter, revealNihilistLetter } from "../utils/letterRevealers";
...
export const useHintSystem = (
  ...
  const getCompleteColumnarHintWrapper = useCallback(
    (quote: QuoteData): string | null => {
      ...
      return getCompleteColumnarHint(quote, activeHints, quoteIndex, ensureCribWordForIndex);
    },
  ...
  const revealRandomLetter = useCallback(
    (questionIndex: number) => {
      ...
      const correctPlainLetter = getPlainLetterForCipherWrapper(quote, randomCipherLetter);
