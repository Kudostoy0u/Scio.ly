...
export function createQuestionFromQuote(
	...
	const questionEntry = {
		...
		kShift:
			cipherResult.kShift ||
			(normalizedCipherType.includes("K3") ? 1 : undefined),
		...
		portaKeyword:
			cipherResult.portaKeyword || cipherResult.keyword || undefined,
		...
		columnarKey:
			normalizedCipherType === "Complete Columnar"
				? cipherResult.key
				: undefined,
	...
	questionEntry.points = Math.max(
		5,
		Math.round(5 + 25 * questionEntry.difficulty),
	);
