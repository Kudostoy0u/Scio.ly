...
describe('computeCipherDifficulty', () => {
  ...
  it('handles Baconian binary type adjustments', () => {
    ...
    const easy = computeCipherDifficulty({ cipherType: 'Baconian', quote: 'HELLO WORLD', baconianBinaryType: 'A/B' });
    const harder = computeCipherDifficulty({ cipherType: 'Baconian', quote: 'HELLO WORLD', baconianBinaryType: 'Odd/Even' });
  ...
  it('caps difficulty between 0.1 and 0.98', () => {
    ...
    const veryLong = computeCipherDifficulty({ cipherType: 'Random Xenocrypt', quote: 'X'.repeat(1000) });
