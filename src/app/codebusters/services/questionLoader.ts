...
function restoreSubmittedTest(
	...
	if (testAlreadySubmitted && existingQuotes) {
		try {
			...
			logger.error(
				"Failed to restore submitted test, continuing with fresh load",
				e,
			);
		}
	}
...
const loadWordBankFromQuotes = (
	...
	if (allWords.length > 0) {
		...
		logger.log(
			`🔍 Extracted ${allWords.length} words from quotes for cryptarithm generation`,
		);
	}
};
...
const processQuestions = async (
	...
	const questionCount =
		Number.parseInt(
			(testParams.questionCount as string) || String(preferences.questionCount),
		) || preferences.questionCount;
	let cipherTypes = (
		((testParams.cipherTypes || testParams.subtopics) as string[]) || []
