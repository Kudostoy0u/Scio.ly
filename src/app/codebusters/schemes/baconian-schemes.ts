...
export function selectRandomScheme(): BaconianScheme {
  ...
  const categorySchemes =
    baconianSchemes.schemes[selectedCategory as keyof typeof baconianSchemes.schemes];
  ...
  const randomScheme = categorySchemes[Math.floor(Math.random() * categorySchemes.length)];
  if (!randomScheme) {
