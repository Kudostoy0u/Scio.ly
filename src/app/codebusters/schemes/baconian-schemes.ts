...
export function selectRandomScheme(): BaconianScheme {
	...
	for (const [category, prob] of Object.entries(
		baconianSchemes.probabilities,
	)) {
		cumulativeProb += prob;
	...
	const categorySchemes =
		baconianSchemes.schemes[
			selectedCategory as keyof typeof baconianSchemes.schemes
		];
	if (!categorySchemes || categorySchemes.length === 0) {
		// Fallback to traditional if category is empty
		const traditional = baconianSchemes.schemes.traditional;
		const fallback = traditional[0];
		if (!fallback) {
			throw new Error("No baconian schemes available");
		}
		return fallback;
	}
	const randomScheme =
		categorySchemes[Math.floor(Math.random() * categorySchemes.length)];
	if (!randomScheme) {
		// Fallback to traditional if random scheme is undefined
		const traditional = baconianSchemes.schemes.traditional;
		const fallback = traditional[0];
		if (!fallback) {
			throw new Error("No baconian schemes available");
