...
function getCachedZeroLetter(
	cacheKey: string,
	zeroSet: string,
	char: string,
): void {
	if (zeroSet && zeroSet.length > 0) {
		categoryLetterCache.set(
			cacheKey,
			zeroSet[Math.floor(Math.random() * zeroSet.length)] || char,
		);
	}
}

// Helper function to get cached or generate letter for one set
function getCachedOneLetter(
	cacheKey: string,
	oneSet: string,
	char: string,
): void {
	if (oneSet && oneSet.length > 0) {
		categoryLetterCache.set(
			cacheKey,
			oneSet[Math.floor(Math.random() * oneSet.length)] || char,
		);
	}
}

// Helper function to process a single character in category type
function processCategoryChar(
	char: string,
	index: number,
...
export function getCssClassForFormatting(
	char: string,
	scheme: BaconianScheme,
): string {
