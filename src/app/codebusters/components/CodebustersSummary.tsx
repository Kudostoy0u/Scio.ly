...
function NonCompactCodebustersSummary({
  items,
  cipherTypes,
  darkMode,
}: {
  ...
  return (
    <div className="sticky top-4 z-10 w-full max-w-[90vw] md:max-w-6xl mx-auto mb-6">
      ...
      >
        <h2
          className={`text-lg md:text-xl font-bold mb-3 md:mb-4 text-center ${darkMode ? "text-white" : "text-gray-900"}`}
        >
          Test Summary
        </h2>
...
function CompactCodebustersSummary({
  items,
  darkMode,
}: {
  items: SummaryItem[];
...
function MobileCompactCodebustersSummary({
  items,
  darkMode,
}: {
  items: SummaryItem[];
...
export default function CodebustersSummary({
  quotes,
  darkMode,
  hintedLetters = {},
  _hintCounts = {},
  ...
  const accuracyPercentage =
    totalPointsAttempted > 0 ? Math.round((totalPointsEarned / totalPointsAttempted) * 100) : "N/A";
  ...
  const nonCompactOpacity =
    scrollY > fadeThreshold ? Math.max(0.3, 1 - (scrollY - fadeThreshold) / fadeRange) : 1;
  const compactOpacity =
    scrollY > fadeThreshold ? Math.min(1, (scrollY - fadeThreshold) / fadeRange) : 0;
