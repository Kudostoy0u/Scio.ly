...
export function buildMappingFromKeyArray(key: string[]): {
	[key: string]: string;
} {
	const mapping: { [key: string]: string } = {};
	for (let i = 0; i < 26; i++) {
...
export function buildCaesarMappingWithShift(shift: number): {
	[key: string]: string;
} {
	const mapping: { [key: string]: string } = {};
	for (let i = 0; i < 26; i++) {
...
export function buildCaesarMappingWithoutShift(
	...
	for (
		let i = 0;
		i < Math.min(ciphertext.length, expectedPlaintext.length);
		i++
	) {
...
export function buildAffineMapping(
	a: number,
	b: number,
): { [key: string]: string } {
	const mapping: { [key: string]: string } = {};
...
export function buildKeywordBasedMapping(
	...
	if (quote.plainAlphabet && quote.cipherAlphabet) {
		return buildMappingFromAlphabets(
...
export function buildOtherCipherMapping(
	...
	if (
		(cipherType === "Xenocrypt" ||
			["Nihilist", "Fractionated Morse", "Complete Columnar"].includes(
				cipherType,
			)) &&
		quote.key
	) {
...
export function buildCorrectMappingForSubstitution(
	...
	const keywordMapping = buildKeywordBasedMapping(
		cipherType,
		quote,
		quoteIndex,
		quotes,
