		...
		<div className="font-mono">
			{currentQuote?.askForKeyword && (
				...
				>
					...
					<div>
						The decrypted text will not be graded for this quote. Instead,
						figure out the <strong>key used to encode the cipher</strong>.
					</div>
					<div className="text-sm mt-1 opacity-80">
						Enter the key in the input boxes below.
					</div>
				</div>
			)}
				...
				<div className="mb-6">
					...
					<div className="flex flex-wrap gap-2">
						{Array.from({ length: currentQuote?.key?.length || 0 }, (_, i) => {
							...
							const isCorrect =
								isTestSubmitted &&
								currentValue.toUpperCase() === expectedValue.toUpperCase();
			...
			<FrequencyTable
				...
				onNoteChange={(letter, note) =>
					onFrequencyNoteChange(quoteIndex, letter, note)
				}
				quoteIndex={quoteIndex}
			/>
			<ReplacementTable
