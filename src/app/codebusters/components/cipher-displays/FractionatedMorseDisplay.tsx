  ...
  const calculatePlaintextLetters = (triplets: string[]): string[] => {
    ...
    for (let i = 0; i < morseString.length; i++) {
      ...
      if (char === "x") {
        ...
        if (
          i + 1 < morseString.length &&
          morseString[i + 1] === "x" &&
          i + 2 < morseString.length
        ) {
      ...
      <div className="flex flex-wrap gap-y-8 text-sm sm:text-base break-words whitespace-pre-wrap">
        {(() => {
          ...
          text.split("").forEach((char, index) => {
            if (/[A-Z]/.test(char)) {
              const triplet =
                isTestSubmitted && correctMapping[char]
                  ? correctMapping[char]
                  : solution?.[char] || "";
              if (triplet.length === 3) {
                ...
                const placeholder =
                  triplet.length === 0 ? "xxx" : triplet + "x".repeat(3 - triplet.length);
          ...
          return text.split("").map((char, i) => {
            ...
            const plaintextLetter =
              isLetter && !incompleteTriplets.has(i) ? plaintextLetters[i] || "" : "";
                  ...
                  <div className="relative h-12 sm:h-14">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        logger.log("Cipher input change:", {
                          char,
                          finalValue,
                      ...
                      {isTestSubmitted ? (
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (
                              existingLetters.includes(newLetter) &&
                              newLetter !== replacementValue
                            ) {
                              logger.log("Letter already used:", newLetter);
                              return;
                            }

                            logger.log("Updating replacement table:", {
                              triplet,
                              newLetter,
                              existingLetters,
                            });
