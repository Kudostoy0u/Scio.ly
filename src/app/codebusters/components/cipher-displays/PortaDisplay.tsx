...
export const PortaDisplay = ({
	...
	return (
		<div className="font-mono">
			{/* Keyword display */}
			<div
				className={`mb-4 p-3 rounded ${darkMode ? "bg-gray-700/50" : "bg-gray-50"}`}
			>
				<p
			...
			>
				...
				>
					...
					<span
						className={`transform transition-transform ${showTable ? "rotate-90" : ""}`}
					>
						▸
					</span>
				</button>
								...
								>
									{row.row.split("").map((char, charIndex) => (
										<span
											...
											onMouseEnter={() =>
												setHoveredChar({ rowIndex: index, charIndex })
											}
											onMouseLeave={() => setHoveredChar(null)}
										>
											{char}
											{hoveredChar?.rowIndex === index &&
												hoveredChar?.charIndex === charIndex && (
													<div
