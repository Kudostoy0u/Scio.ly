...
export const CryptarithmDisplay: React.FC<CryptarithmDisplayProps> = ({
	...
	const verticalDisplay = React.useMemo(() => {
		...
		const { line1, line2, line3, separator } = buildVerticalLines(
			w1,
			w2,
			w3,
			maxLen,
				...
				>
					...
					<div className="flex flex-wrap gap-2 justify-center">
						{inlineDigits.digits.map((digit, i) => {
							const position = i;
							const isCorrect =
								correctMapping[position] === (solution?.[position] || "");
							const isHinted =
								_quotes[quoteIndex]?.cryptarithmHinted?.[position] === true;
							const isSameDigitFocused =
								focusedDigit !== null && focusedDigit === digit;
