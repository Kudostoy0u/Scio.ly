...
export const CheckerboardDisplay = ({
  ...
  const [gridValues, setGridValues] = useState<string[][]>(() => Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => '')));
  ...
  const findColIndex = (ch: string): number => {
    ...
    for (let i = 0; i < 5; i++) {
      const label = colLabels[i].toUpperCase();
      if (label === 'I/J') {
        if (target === 'I') return i;
      } else if (normalizeIJ(label) === target) return i;
    }
    return -1;
  };
  ...
  const findRowIndex = (ch: string): number => {
    const target = normalizeIJ(ch.toUpperCase());
    for (let i = 0; i < 5; i++) {
      const label = rowLabels[i].toUpperCase();
      if (label === 'I/J') {
        if (target === 'I') return i;
      } else if (normalizeIJ(label) === target) return i;
    }
  ...
  const syncGridFromSolutions = (rows: string[] = rowLabels, cols: string[] = colLabels) => {
    ...
    const findCol = (ch: string) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (label === 'I/J') { if (target === 'I') return i; }
        else if (normalizeIJ(label || '') === target) return i;
      }
      return -1;
    };
    ...
    setGridValues(() => {
      ...
      tokens.forEach((tok, idx) => {
        ...
        if (val && ri !== -1 && ci !== -1) next[ri][ci] = val;
      });
      return next;
    });
  };
      ...
      <div className={`mb-4 p-3 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
        <div className="grid grid-cols-2 items-center text-sm">
          ...
          <div className="text-xs text-right">
            <span className="font-semibold">Note:</span> {usesIJ ? 'Using 25-letter alphabet (I/J same).' : 'Using 26-letter alphabet.'}
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-col md:flex-row md:gap-6 items-start">
        ...
        <div className="md:flex-[4] md:min-w-0">
          <div className="flex flex-wrap gap-2">
            {tokens.map((tok, idx) => (
              <div key={idx} className={`flex flex-col items-center ${blockEnd.has(idx) ? 'mr-6 md:mr-10' : ''}`}>
                ...
                <input
                  ...
                  className={`w-8 h-8 text-center border rounded text-sm outline-none focus:outline-none ${
                    isTestSubmitted
                      ? (() => {
                          const isHinted = Boolean((quote as any).checkerboardHinted?.[idx]);
                          if (isHinted) return 'bg-transparent border-yellow-500';
                          return 'bg-transparent ' + (correctMapping[idx] === (solution?.[idx] || '').toUpperCase() ? 'border-green-500' : 'border-red-500');
                        })()
                      : darkMode
                ...
                {isTestSubmitted && (
                  <div className={`mt-1 text-[10px] ${
                    ...
                    {((solution?.[idx] || '').length === 0 || (solution?.[idx] || '').toUpperCase() !== (correctMapping[idx] || ''))
          ...
          <div className="inline-block mx-auto md:mx-0">
            <div className="grid grid-cols-6 gap-1">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                isTestSubmitted ? (
                  <div key={`cb-col-${i}`} className={`w-8 h-8 text-center border rounded text-xs flex items-center justify-center ${darkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-300'}`}>
                    {(() => {
                      const expected = (colKey[i] || '').toUpperCase();
                      const user = (colLabels[i] || '').toUpperCase();
                      if (!user) return <span className="text-red-600 font-mono">{expected}</span>;
                      if (user === expected) return <span className="text-green-600 font-mono">{expected}</span>;
                      return (
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      syncGridFromSolutions(rowLabels, (() => { const n=[...colLabels]; n[i]=normalized; return n; })());
                    }}
                    onKeyDown={(e) => {
                      if (e.key === 'Backspace' && colLabels[i] === 'I/J') {
                        e.preventDefault();
                        setColLabels(_prev => { const n=[...colLabels]; n[i]=''; return n; });
                      }
                    }}
              ...
              {Array.from({ length: 5 }).map((_, ri) => (
                <React.Fragment key={`cb-row-${ri}`}>
                  {isTestSubmitted ? (
                    <div className={`w-8 h-8 text-center border rounded text-xs flex items-center justify-center ${darkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-300'}`}>
                      {(() => {
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        syncGridFromSolutions((() => { const n=[...rowLabels]; n[ri]=normalized; return n; })(), colLabels);
                      }}
                      onKeyDown={(e) => {
                        if (e.key === 'Backspace' && rowLabels[ri] === 'I/J') {
                          e.preventDefault();
                          setRowLabels(_prev => { const n=[...rowLabels]; n[ri]=''; return n; });
                        }
                      }}
                      className={`w-8 h-8 text-center border rounded text-xs outline-none focus:outline-none ring-1 ring-blue-200 ${darkMode ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-800'}`}
                    />
                  ...
                  {Array.from({ length: 5 }).map((_, ci) => (
                    isTestSubmitted ? (
                      <div key={`cb-cell-${ri}-${ci}`} className={`w-8 h-8 border rounded-sm flex items-center justify-center ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}>
                        {(() => {
                          // Compute expected letter from Polybius key using 25-letter alphabet with I/J combined
                          const normalizeIJD = (s: string) => (s || '').toUpperCase().replace(/J/g, 'I');
                        ...
                        onChange={(e) => {
                          ...
                          if (rLab && cLab) {
                            ...
                            tokensLocal.forEach((tok, idx) => {
                              if (normalizeToken(tok) === normalizeToken(desiredToken)) onSolutionChange(quoteIndex, idx, val);
                            });
                          }
                        }}
                        onKeyDown={(e) => {
                          if (e.key === 'Backspace' && (gridValues[ri]?.[ci] || '') === 'I/J') {
                            e.preventDefault();
                            setGridValues(prev => { const n = prev.map(r => [...r]); n[ri][ci] = ''; return n; });
                          }
                        }}
      ...
      {isTestSubmitted && (
        <div className={`mt-4 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4">
            <div><span className="font-semibold">Row key: </span><span className="font-mono">{rowKey}</span></div>
            <div><span className="font-semibold">Column key: </span><span className="font-mono">{colKey}</span></div>
          </div>
        </div>
      )}
    </div>
