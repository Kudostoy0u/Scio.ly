...
const buildCorrectMapping = (text: string, originalQuote: string): { [key: number]: string } => {
...
export const HillDisplay = ({
  ...
  const paddingPositions = useMemo(() => {
    ...
    return calculatePaddingPositions(cipherLetterIndices, cleanPlainLength, matrixSize);
  ...
  const getMatrixInputClassName = (
    ...
    const baseClasses = "w-10 h-10 sm:w-12 sm:h-12 text-center border rounded text-base sm:text-lg";
  ...
  const handleMatrixCellChange = (
    ...
    const baseMatrix =
      solution?.matrix || Array.from({ length: matrixSize }, () => new Array(matrixSize).fill(""));
    ...
    return (
      <div key={`solution-${i}-${j}`} className="flex flex-col items-center">
        <input
          ...
          className={getMatrixInputClassName(isTestSubmitted, isCorrect, darkMode)}
          placeholder="?"
          autoComplete="off"
  ...
  const handleCharacterInputChange = (e: React.ChangeEvent<HTMLInputElement>, i: number) => {
      ...
      <div key={i} className="flex flex-col items-center mx-0.5">
        ...
        <div className="relative h-12 sm:h-14">
          <input
            ...
            className={getCharacterInputClassName(isPadding, isTestSubmitted, isCorrect, darkMode)}
          />
      ...
      <div className="flex flex-wrap gap-y-8 text-sm sm:text-base break-words whitespace-pre-wrap">
        {(() => {
          ...
          return charsWithPositions.map(({ char, position }) => {
            ...
            const correctLetter =
              isTestSubmitted && isLetter ? (correctMapping[position] ?? "") : "";
            const isCorrect = value.toUpperCase() === correctLetter;
            const isPadding = isLetter && isTestSubmitted && paddingPositions.has(position);
