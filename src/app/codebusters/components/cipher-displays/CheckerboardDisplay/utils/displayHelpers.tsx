...
export function getTokenInputClassName(
  ...
  const baseClasses = "w-8 h-8 text-center border rounded text-sm outline-none focus:outline-none";
  if (isTestSubmitted) {
    if (isHinted) {
...
export function renderLabelDisplay(expected: string, user: string): React.ReactNode {
  if (!user) {
