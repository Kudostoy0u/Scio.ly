...
function findUnfilledIndices(allLetters: string, current: Record<number, string>): number[] {
...
function findPositionsToFill(allDigitsArr: string[], targetDigit: string): number[] {
...
function shouldEnableBaconianSync(binaryType: string): boolean {
  ...
  return !(binaryType === "Vowels/Consonants" || binaryType === "Odd/Even" || hasMultipleValues);
}
...
function processCryptarithmHint(
  ...
  index: number,
  quotes: QuoteData[],
  handleCryptarithmSolutionChange: (index: number, pos: number, value: string) => void
): void {
...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const handleHintButtonClick = (): void => {
    ...
    if (item.cipherType === "Cryptarithm") {
      processCryptarithmHint(item, index, quotes, handleCryptarithmSolutionChange);
    }
  ...
  const renderBaconianSyncButton = (): React.ReactNode | null => {
    ...
    const getButtonClassName = (): string => {
      const baseClasses = "px-2 py-1 rounded text-xs border transition-all duration-200";
      ...
      const disabledClasses = isTestSubmitted ? "opacity-50 cursor-not-allowed" : "hover:scale-105";
    ...
    const getButtonTitle = (): string => {
      ...
      return baconianSyncEnabled ? "Disable input syncing" : "Enable input syncing";
    };
  ...
  const renderHintCard = (): React.ReactNode | null => {
    if (!activeHints[index] || isTestSubmitted || item.cipherType === "Cryptarithm") {
      return null;
    }
    return (
      <div
  ...
  const renderDifficultyBar = (): React.ReactNode => {
    ...
    const difficultyColor =
      difficulty >= 0.66 ? "bg-red-500" : difficulty >= 0.33 ? "bg-yellow-500" : "bg-green-500";
    return (
