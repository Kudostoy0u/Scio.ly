...
export const encryptRandomAristocrat = (
	text: string,
): { encrypted: string; key: string } => {
	const key = generateDerangementFromAlphabet(ALPHABET_26);
	const encrypted = text
		.toUpperCase()
		.replace(/[A-Z]/g, (ch) => key[letterToNumber(ch)] || ch);
	return { encrypted, key };
};

export const encryptRandomPatristocrat = (
	text: string,
): { encrypted: string; key: string } => {
	const key = generateDerangementFromAlphabet(ALPHABET_26);
...
export const encryptRandomXenocrypt = (
	text: string,
): { encrypted: string; key: string } => {
	const key = generateDerangementFromAlphabet(ALPHABET_27_INDEX_ORDER);
	const encrypted = text
		.toUpperCase()
		.replace(/[A-ZÑ]/g, (ch) => key[letterToNumber(ch)] || ch);
	return { encrypted, key };
};
