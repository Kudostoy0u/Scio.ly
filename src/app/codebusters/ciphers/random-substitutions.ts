...
import { ALPHABET_26, ALPHABET_27_INDEX_ORDER, generateDerangementFromAlphabet } from '../utils/substitution';
...
export const encryptRandomPatristocrat = (text: string): { encrypted: string; key: string } => {
    ...
    const groups: string[] = [];
    for (let i = 0; i < letters.length; i += 5) groups.push(letters.slice(i, i + 5).join(''));
