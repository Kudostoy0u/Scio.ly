...
export const encryptCryptarithm = (_text: string): CryptarithmResult => {
  ...
  const pickWordFn = (exclude: Set<string> = new Set()) => pickWord(uniqueWords, exclude);
  ...
  const generateCryptarithm = (): {
    ...
    if (isSubtraction) {
      const additionResult = attemptGenerateCryptarithm("+", attempts, uniqueWords, pickWordFn);
      if (additionResult) {
