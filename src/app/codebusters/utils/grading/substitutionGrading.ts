...
export const checkSubstitutionCorrectness = (
	...
	const storedResult = checkStoredMapCorrectness(
		quote,
		cipherLetter,
		userAnswer,
	);
...
export const gradeSubstitutionCipher = (
	...
	if (quote.solution && Object.keys(quote.solution).length > 0) {
		const allUniqueLetters = [
			...new Set(quote.encrypted.replace(/[^A-Z]/g, "")),
		];
		const nonHintedLetters = allUniqueLetters.filter(
			(c) => !hintedLetters[quoteIndex]?.[c],
		);
