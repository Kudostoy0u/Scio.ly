...
export const getBaconianStartIndex = (letterOnlyPlain: string, quote: QuoteData): number => {
...
export const revealBaconianLetter = (
  ...
  const newQuotes = quotes.map((q, idx) => {
    ...
    return updateBaconianQuote(q, groupTokens, groupParsedIdxs, targetToken, correctPlainLetter);
  });
  if (newQuotes) {
    setQuotes(newQuotes);
  }
