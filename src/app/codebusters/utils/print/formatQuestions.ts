...
const formatReplacementTable = (quote: QuoteData): string => {
	...
	tableHtml +=
		'<div style="font-weight: bold; margin-bottom: 5px;">Replacement Table</div>';
...
const formatHillMatrix = (matrix: number[][]): string => {
	...
	matrixHtml +=
		'<div style="font-weight: bold; margin-bottom: 5px;">Hill Cipher Matrix</div>';
...
const formatFractionatedMorseCipher = (quote: QuoteData): string => {
	...
	if (quote.fractionationTable) {
		html +=
			'<div style="margin-top: 10px; font-size: 12px;">Fractionation Table provided</div>';
	}
	html += formatReplacementTable(quote);
	return html;
};
