...
import {
	...
	gradeColumnar,
	gradeNihilist,
} from "./grading/otherCipherGrading";
...
const routeCipherTypeGrading = (
	...
	const cipherTypeMap: { [key: string]: () => GradingResult } = {
		...
		"Fractionated Morse": () =>
			gradeFractionatedMorse(quote, quoteIndex, hintedLetters),
...
const getUnitLabel = (cipherType: string, result: GradingResult): string => {
	...
	return plaintextCiphers.includes(cipherType)
...
export function calculateCipherGrade(
	...
	const questionPointValue = resolveQuestionPoints(
		quote,
		quoteIndex,
		questionPoints,
	);
	...
	if (isSubstitutionCipher(quote.cipherType)) {
		return gradeSubstitutionCipher(
			quote,
			quoteIndex,
			hintedLetters,
			questionPointValue,
	...
	const result = routeCipherTypeGrading(
		quote,
		quoteIndex,
		hintedLetters,
		questionPointValue,
	...
	const attemptedScore =
		result.totalInputs > 0
			? (result.filledInputs / result.totalInputs) * questionPointValue
			: 0;
	const score =
		result.filledInputs > 0
			? (result.correctInputs / result.filledInputs) * attemptedScore
			: 0;
