...
export const findDerangementShift = (
...
export const applySubstitutionPatristocrat = (
	...
	const cleaned = text
		.toUpperCase()
		.replace(
			new RegExp(`[^${lettersRegex.source.replace(/[\\/]/g, "")}]`, "g"),
			"",
		);
...
export const getLetterFrequencies = (
	text: string,
): { [key: string]: number } => {
	const frequencies: { [key: string]: number } = {};
	const upper = text.toUpperCase();
	const isXeno = upper.includes("Ñ");
	const alphabet = isXeno
	...
	for (const ch of upper) {
		if (
			isXeno ? XENOCRYPT_LETTER_REGEX.test(ch) : STANDARD_LETTER_REGEX.test(ch)
		) {
