...
export const findDerangementShift = (plainAlphabet: string, cipherAlphabet: string): number => {
    ...
    for (let s = 1; s < len; s++) {
        let ok = true;
        for (let i = 0; i < len; i++) {
            if (plainAlphabet[i] === cipherAlphabet[(i + s) % len]) { ok = false; break; }
        }
        if (ok) return s;
    }
    return 1;
...
export const getRandomDerangementShift = (plainAlphabet: string, cipherAlphabet: string): number => {
    const len = Math.min(plainAlphabet.length, cipherAlphabet.length);
...
export const buildSubstitutionMap = (plainAlphabet: string, cipherAlphabet: string): Record<string, string> => {
    const len = Math.min(plainAlphabet.length, cipherAlphabet.length);
...
export const applySubstitutionAristocrat = (text: string, map: Record<string, string>, lettersRegex: RegExp = /[A-Z]/g): string => {
    return text.toUpperCase().replace(lettersRegex, ch => map[ch] || ch);
};

export const applySubstitutionPatristocrat = (text: string, map: Record<string, string>, lettersRegex: RegExp = /[A-Z]/g): string => {
    const cleaned = text.toUpperCase().replace(new RegExp(`[^${lettersRegex.source.replace(/[\\/]/g, '')}]`, 'g'), '');
    const letters = cleaned.replace(/[^A-ZÑ]/g, '').split('').map(ch => map[ch] || ch);
    const grouped: string[] = [];
    for (let i = 0; i < letters.length; i += 5) grouped.push(letters.slice(i, i + 5).join(''));
    return grouped.join(' ');
};
...
export const getLetterFrequencies = (text: string): { [key: string]: number } => {
    ...
    for (const l of alphabet) frequencies[l] = 0;
    for (const ch of upper) {
        if (isXeno ? /[A-ZÑ]/.test(ch) : /[A-Z]/.test(ch)) frequencies[ch]++;
    }
    if (isXeno && frequencies['Ñ'] === undefined) frequencies['Ñ'] = 0;
    return frequencies;
};
