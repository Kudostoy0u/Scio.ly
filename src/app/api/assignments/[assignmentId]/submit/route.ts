...
function calculatePoints(
  ...
  const calcCorrectAnswers =
    Math.round(
      (calcEarnedPoints / Math.max(1, calcTotalPoints)) * Object.keys(answers || {}).length
    ) || 0;
...
export async function POST(
  ...
  try {
    ...
    const { calculatedTotalPoints, calculatedEarnedPoints, calculatedCorrectAnswers } =
      calculatePoints(isDynamicCodebusters, answers, body, totalPoints, assignment.maxPoints);
