  ...
  try {
    ...
    if (shouldRemove) {
      ...
      try {
        ...
        if (questionId) {
          try {
            logger.dev.db("DELETE", "questions", "DELETE FROM questions WHERE id = ?", [
              questionId,
            ]);
          ...
          if (question.tournament) {
            conditions.push(eq(questionsTable.tournament, String(question.tournament)));
          }
          if (question.division) {
            conditions.push(eq(questionsTable.division, String(question.division)));
          }
