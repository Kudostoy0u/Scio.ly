...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.dev.request('POST', '/api/report/remove', undefined, Object.fromEntries(request.headers.entries()));
    ...
    logger.dev.structured('info', 'Report remove request processing started', {
      ...
      questionText: question.question ? String(question.question).substring(0, 100) + '...' : 'No question text',
    ...
    if (geminiService.isAvailable()) {
      ...
      try {
        const result = await geminiService.analyzeQuestionForRemoval(question, event);
        const aiAnalysisDuration = Date.now() - aiAnalysisStartTime;

        logger.info('Gemini AI question removal analysis response received');
        logger.dev.ai('Gemini', 'analyzeQuestionForRemoval', question, result, aiAnalysisDuration);

        shouldRemove = Boolean((result as any).shouldRemove);
    ...
    if (shouldRemove) {
      ...
      try {
        ...
        logger.dev.db('INSERT', 'blacklists', 'INSERT INTO blacklists (event, questionData) VALUES (?, ?)', [event, questionDataJSON]);
        await db
        ...
        if (questionId) {
          try {
            logger.dev.db('DELETE', 'questions', 'DELETE FROM questions WHERE id = ?', [questionId]);
            await db.delete(questionsTable).where(eq(questionsTable.id, questionId));
            logger.info('Removed question from main table by id');
            logger.dev.structured('info', 'Question deleted by ID', { questionId });
          } catch (error) {
            logger.warn('Question might not exist in main table (id path):', error);
            logger.dev.error('Question deletion by ID failed', error as Error, { questionId });
          }
        } else {
          ...
          logger.dev.structured('info', 'Attempting content-based question deletion', {
            conditions: {
          ...
          try {
            logger.dev.db('SELECT', 'questions', 'SELECT id FROM questions WHERE ...', []);
            ...
            if (targetId) {
              logger.dev.db('DELETE', 'questions', 'DELETE FROM questions WHERE id = ?', [targetId]);
              ...
              logger.dev.structured('info', 'Question deleted by content', { targetId });
            } else {
              logger.warn('Could not locate question in main table by content');
              logger.dev.structured('warn', 'Question not found for content-based deletion', {
