...
export async function POST(request: NextRequest) {
	try {
		...
		logger.debug("Report edit request", {
			bypass,
		...
		if (aiSuggestion) {
			try {
				...
				const aiOpts = Array.isArray(aiSuggestion.options)
					...
					: undefined;
				const aiAns = Array.isArray(aiSuggestion.answers)
					? aiSuggestion.answers.map(String)
					: [];
				...
				const matches =
					editedQ === aiQ &&
					arraysEqual(editedOpts, aiOpts) &&
					arraysEqual(editedAnsText, aiAns);
				logger.debug("AI bypass verification", {
					matches,
					editedAnsText,
					aiAns,
				});
		...
		if (canBypass) {
			...
			try {
				...
				logger.info(`AI Decision: ${isValid ? "accepted" : "rejected"}`, {
					reason: aiReason,
				});
			} catch (error) {
				isValid = false;
		...
		if (isValid) {
			...
			try {
				...
				let targetId: string | undefined =
					(originalQuestion.id as string | undefined) ||
					(editedQuestion.id as string | undefined);
				if (!targetId) {
					...
					if (originalQuestion.tournament) {
						conditions.push(
							eq(
								questionsTable.tournament,
								String(originalQuestion.tournament),
							),
						);
					}
					if (originalQuestion.division) {
						conditions.push(
							eq(questionsTable.division, String(originalQuestion.division)),
						);
					}
					...
					if (!targetId) {
						const cond2: SQL[] = [
							eq(
								questionsTable.question,
								String(editedQuestion.question || ""),
							),
							eq(questionsTable.event, String(event)),
						];
						if (editedQuestion.tournament) {
							cond2.push(
								eq(
									questionsTable.tournament,
									String(editedQuestion.tournament),
								),
							);
						}
						if (editedQuestion.division) {
							cond2.push(
								eq(questionsTable.division, String(editedQuestion.division)),
							);
						}
