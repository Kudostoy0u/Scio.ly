...
describe("/api/gemini/analyze-question", () => {
	...
	describe("Request Validation", () => {
		it("should return 400 for missing question field", async () => {
			const request = mockRequest({} as AnalyzeQuestionRequest);
		...
		it("should return 500 for invalid JSON", async () => {
			const request = new NextRequest(
				"http://localhost:3000/api/gemini/analyze-question",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: "invalid json",
	...
	describe("Question Analysis", () => {
		...
		it("should handle analysis errors", async () => {
			(geminiService.isAvailable as Mock).mockReturnValue(true);
			(geminiService.analyzeQuestion as Mock).mockRejectedValue(
	...
	describe("Performance and Timeout", () => {
		...
		it("should handle concurrent analysis requests", async () => {
			...
			const requests = new Array(5)
				.fill(null)
				.map(() => mockRequest({ question: mockQuestion }));
