...
describe("POST /api/teams/[teamId]/roster/invite", () => {
	...
	it("should create roster link invitation successfully", async () => {
		...
		expect(
			mockNotificationSyncService.syncNotificationToSupabase,
	...
	it("should return 400 if subteam ID is invalid UUID", async () => {
		...
		expect(Array.isArray(data.details)).toBe(true);
		expect(
			data.details.some((detail: string) => SUBTEAM_ID_REGEX.test(detail)),
		).toBe(true);
	});
	...
	it("should return 400 if invitation already sent", async () => {
		...
		expect(Array.isArray(data.details)).toBe(true);
		expect(
			data.details.some((detail: string) => INVITATION_SENT_REGEX.test(detail)),
		).toBe(true);
	});
	...
	it("should update existing declined invitation", async () => {
		...
		mockDbPg.update.mockImplementationOnce(() =>
			createMockDrizzleUpdate(mockInvitation),
		);
		mockDbPg.insert.mockImplementationOnce(() =>
			createMockDrizzleInsert(mockNotification),
		);
