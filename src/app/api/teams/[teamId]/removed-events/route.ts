...
export async function POST(
	...
	try {
		...
		try {
			...
			return handleError(
				error,
				"POST /api/teams/[teamId]/removed-events - validation",
			);
		}
		...
		await dbPg
			...
			.onConflictDoUpdate({
				target: [
					newTeamRemovedEvents.teamUnitId,
					newTeamRemovedEvents.eventName,
				],
				set: {
					conflictBlock,
					removedBy: user.id,
...
export async function DELETE(
	...
	try {
		...
		try {
			...
			return handleError(
				error,
				"DELETE /api/teams/[teamId]/removed-events - validation",
			);
		}
