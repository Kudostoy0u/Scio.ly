...
export async function GET(
	...
	try {
		...
		for (const member of members) {
			...
			const email =
				userProfile?.email ||
				`user-${member.userId.substring(0, 8)}@example.com`;
			...
			if (existingMember) {
				...
				for (const [teamUnitId, events] of userRosterData) {
					const subteamInfo = {
						id: teamUnitId,
						name:
							linkedRosterResult.find((r) => r.teamUnitId === teamUnitId)
								?.description ||
							`Team ${linkedRosterResult.find((r) => r.teamUnitId === teamUnitId)?.teamId}`,
						description: linkedRosterResult.find(
							(r) => r.teamUnitId === teamUnitId,
						)?.description,
					...
					if (
						...
						existingMember.subteams?.length === 1
					) {
						existingMember.subteam = {
							id: subteamInfo.id,
							name: subteamInfo.name,
		...
		for (const rosterMember of unlinkedRosterResult) {
			...
			const studentRosterData = unlinkedRosterByStudent.get(
				rosterMember.studentName,
			);
			...
			if (rosterMember.eventName) {
				studentRosterData
					.get(rosterMember.teamUnitId)
					?.push(rosterMember.eventName);
			}
		}
