	...
	try {
						...
						const [analyticsResult] = await dbPg
							.select({
								...
								completion_time_seconds:
									newTeamAssignmentAnalytics.completionTimeSeconds,
			...
			const responsesResult = await dbPg
				...
				.innerJoin(
					...
					eq(
						...
						newTeamAssignmentSubmissions.id,
					),
				)
		...
		return NextResponse.json({
			assignment: {
				...
				submitted_count: rosterWithSubmissions.filter((r) => r.submission)
					.length,
				graded_count: rosterWithSubmissions.filter(
