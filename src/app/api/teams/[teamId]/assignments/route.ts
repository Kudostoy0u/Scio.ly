      ...
      assignmentsResult.map(async (assignment) => {
        ...
        const submissionCountResult = await dbPg
          ...
          .where(
            and(
              eq(newTeamAssignmentSubmissions.assignmentId, assignment.id),
              inArray(newTeamAssignmentSubmissions.status, ["submitted", "graded"])
            )
          );
...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      return handleError(error, "POST /api/teams/[teamId]/assignments - validation");
    }
    ...
    if (!hasLeadership) {
      return handleForbiddenError("Only captains and co-captains can create assignments");
    }
    ...
    if (!targetTeamId) {
      return NextResponse.json({ error: "No valid team unit found" }, { status: 400 });
    }
