...
import { DELETE, PUT } from '../events/[eventId]/route';
...
describe('/api/teams/calendar/events/[eventId]', () => {
  ...
  describe('DELETE /api/teams/calendar/events/[eventId]', () => {
    ...
    it('returns 404 for non-existent event', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/teams/calendar/events/non-existent', {
        method: 'DELETE',
      });

      const response = await DELETE(request, { params: Promise.resolve({ eventId: 'non-existent' }) });
      const data = await response.json();
  ...
  describe.skip('PUT /api/teams/calendar/events/[eventId]', () => {
