...
import { createShareCode, loadTeamDataByCode, saveTeamData, loadTeamData } from '@/lib/db/teams';
  ...
  try {
    ...
    if (action === 'create') {
      ...
      const teamData = {
        ...
        division,
        teams,
        captainCode: existing?.captainCode || `CAPT_${school}_${division}`,
        userCode: existing?.userCode || `USER_${school}_${division}`
      };
