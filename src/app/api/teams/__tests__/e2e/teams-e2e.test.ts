...
describe("Teams E2E Tests", () => {
	...
	describe("Team Creation Flow", () => {
		it("should create a team with default subteam", () => {
			...
			const membership = mockDb.memberships.get(
				`${creator.id}:${team.subteamId}`,
			);
	...
	describe("Authorization Checks", () => {
		it("should enforce captain-only operations", () => {
			...
			const creatorMembership = mockDb.memberships.get(
				`${creator.id}:${team.subteamId}`,
			);
			expect(creatorMembership?.role).toBe("captain");

			const memberMembership = mockDb.memberships.get(
				`${member.id}:${team.subteamId}`,
			);
