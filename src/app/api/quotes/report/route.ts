...
import { longquotes as longquotesTable, quotes as quotesTable } from "@/lib/db/schema";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validation = validateFields<QuoteReportRequest>(body, ["quote", "cipherType"]);
    ...
    logger.dev.structured("info", "Quote report request processing started", {
      cipherType,
      quoteId: quote.id,
      quoteText: quote.quote ? `${String(quote.quote).substring(0, 100)}...` : "No quote text",
      author: quote.author,
    ...
    if (geminiService.isAvailable()) {
      ...
      try {
        ...
        logger.dev.ai("Gemini", "validateQuote", quote, result, aiAnalysisDuration);
    ...
    if (action === "remove") {
      ...
      try {
        ...
        if (quoteId && quoteId.startsWith("assignment-") === false) {
          ...
          try {
            ...
            logger.dev.structured("info", "Quote deleted by ID from quotes table", { quoteId });
            deleted = true;
          } catch (error) {
            logger.warn("Quote not found in quotes table by ID, trying longquotes:", error);
            logger.dev.error("Quote deletion by ID failed from quotes table", error as Error, {
              quoteId,
            });

            // Try longquotes table
            try {
              await db.delete(longquotesTable).where(eq(longquotesTable.id, quoteId));
              logger.info("Deleted quote from longquotes table by ID", { quoteId });
              logger.dev.structured("info", "Quote deleted by ID from longquotes table", {
                quoteId,
              });
              deleted = true;
            } catch (error2) {
              logger.warn("Quote not found in longquotes table by ID, trying by content:", error2);
              logger.dev.error(
        ...
        if (!deleted && originalQuoteText && originalAuthor) {
          try {
            ...
            logger.dev.structured("info", "Quote deleted from quotes table by content", {
              cipherType,
            });
            deleted = true;
          } catch (error) {
            logger.warn("Quote not found in quotes table by content, trying longquotes:", error);
            logger.dev.error("Quote deletion failed in quotes table by content", error as Error);
            ...
            try {
              ...
              logger.dev.structured("info", "Quote deleted from longquotes table by content", {
                cipherType,
              });
              deleted = true;
            } catch (error2) {
              logger.error("Could not locate quote in either table for deletion:", error2);
              logger.dev.error(
        ...
        logger.dev.structured("info", "Database operations completed successfully", {
          quoteId: quote.id,
          cipherType,
      ...
      logger.info("Quote edit details", {
        original: { quote: originalQuoteText, author: originalAuthor, language: originalLanguage },
        edited: { quote: editedQuoteText, author: editedAuthor, language: editedLanguage },
      });
      ...
      try {
        ...
        if (quoteId && quoteId.startsWith("assignment-") === false) {
          // Try quotes table first
          try {
            await db.update(quotesTable).set(payload).where(eq(quotesTable.id, quoteId));
            ...
            logger.warn("Quote not found in quotes table by ID, trying longquotes:", error);
            logger.dev.error("Quote update failed in quotes table", error as Error, {
              quoteId,
            });
            ...
            try {
              ...
              logger.dev.structured("info", "Quote updated in longquotes table", {
                quoteId,
                cipherType,
                originalQuote: originalQuoteText,
                editedQuote: editedQuoteText,
              ...
              logger.warn("Quote not found in longquotes table by ID, trying by content:", error2);
              logger.dev.error("Quote update failed in longquotes table", error2 as Error, {
                quoteId,
              });
            }
          }
        ...
        if (!updated && originalQuoteText && originalAuthor) {
          try {
            ...
            logger.dev.structured("info", "Quote updated in quotes table by content", {
              cipherType,
              originalQuote: originalQuoteText,
              editedQuote: editedQuoteText,
              originalAuthor,
            ...
            logger.warn("Quote not found in quotes table by content, trying longquotes:", error);
            logger.dev.error("Quote update failed in quotes table by content", error as Error);
