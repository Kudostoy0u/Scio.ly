...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!email || typeof email !== 'string' || !email.includes('@')) {
      ...
      logger.dev.response(res.status, { error: 'Missing or invalid email' }, Date.now() - startedAt);
      return res;
    }
    ...
    try {
      ...
      logger.dev.error('profile/sync upsert failed', err instanceof Error ? err : new Error(String(err)), {
        id,
        email,
        name,
    ...
    logger.dev.error('profile/sync unhandled error', error instanceof Error ? error : new Error(String(error)));
