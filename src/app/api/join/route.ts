...
export async function POST(req: NextRequest) {
  try {
    ...
    const webhookUrl = "https://discord.com/api/webhooks/1411836447379689657/1AFOH8UU2AHcDZfYi674ABrQFzzk8Aa_GbiU0ZnPwG54xNYr6BfWl_nJj5Kas0yF_Owx"
    if (!webhookUrl) {
      return NextResponse.json({ success: false, message: 'Careers webhook not configured' }, { status: 500 })
    }
    ...
    if (!resp.ok) {
      return NextResponse.json({ success: false, message: 'Failed to submit application' }, { status: 502 })
    }
