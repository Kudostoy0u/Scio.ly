...
const deriveDisplayName = (
  ...
  if (currentName && currentName !== "@unknown" && !currentName.startsWith("User ")) {
    return currentName;
  }
  ...
  if (emailLocal && emailLocal.length > 2 && !HEX_8_CHAR_REGEX.test(emailLocal)) {
    return emailLocal;
  }
  return "";
};
...
const validateInputs = (displayName: string, username: string): string | null => {
  if (!displayName.trim()) {
...
const updateLocalStorage = (userId: string, displayName: string, username: string) => {
  try {
    if (displayName.trim()) {
      SyncLocalStorage.setItem(`scio_display_name_${userId}`, displayName.trim());
...
const verifyPersistedName = async (userId: string) => {
  try {
    const verify = await supabase
      ...
      .maybeSingle();
    if (!verify.error) {
      const dn = (verify.data as { display_name?: string } | null)?.display_name;
      if (dn?.trim()) {
        try {
...
export default function NamePromptModal({
  ...
  useEffect(() => {
    ...
    const emailLocal = currentEmail?.includes("@") ? currentEmail.split("@")[0] : "";
    ...
    if (!username && emailLocal && emailLocal.length > 2 && !HEX_8_CHAR_REGEX.test(emailLocal)) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await syncProfileToAPI(user.id, user.email || currentEmail, displayName, username);
  ...
  const renderSuccessMessage = () => (
    <div className="text-center py-8">
      ...
      >
        <User className={`w-8 h-8 ${darkMode ? "text-green-300" : "text-green-600"}`} />
      </div>
  ...
  const renderWarningBanner = () => (
    ...
    >
      <div className="flex items-start space-x-3">
        <User className={`w-5 h-5 mt-0.5 ${darkMode ? "text-yellow-300" : "text-yellow-600"}`} />
        <div>
        ...
        >
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-full ${darkMode ? "bg-blue-900" : "bg-blue-100"}`}>
              <Edit3 className={`w-5 h-5 ${darkMode ? "text-blue-300" : "text-blue-600"}`} />
            </div>
            <div>
