...
const EditQuestionModal: React.FC<EditQuestionModalProps> = ({
  ...
  useEffect(() => {
    if (question && isOpen) {
      ...
      if (question.imageData || ("imageUrl" in question && question.imageUrl)) {
        setCurrentImageUrl(question.imageData || (question.imageUrl as string));
      }
  ...
  const computeCorrectAnswerIndices = (options: string[], answers: unknown[]): number[] => {
    ...
    const indices = answers
      .map((a) => {
        ...
        logger.log("🔍 [COMPUTE-INDICES] Text match:", {
          searchFor: str,
          foundAt: idx,
          inOptions: lowerOptions,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const finalImageUrl = currentImageUrl || question.imageData || question.imageUrl;

      const editedQuestionData: Question = {
        question: editedQuestion,
        options: isFrq ? undefined : editedOptions.length > 0 ? editedOptions : undefined,
        answers: canEditAnswers ? (isFrq ? [frqAnswer] : correctAnswers) : question.answers,
      ...
      const aiSuggestionPayload =
        suggestions && aiSuggestionsApplied && !suggestionsTampered
          ? {
          ...
          {showSuggestions ? (
            suggestions && (
              ...
              >
                <div className="flex items-start space-x-3 mb-3">
                  <div
                    className={`p-1.5 rounded-full ${darkMode ? "bg-green-600/20" : "bg-green-100"}`}
                  >
                    <svg
            ...
            >
              <div className="flex items-center justify-between">
                ...
                >
                  <Bot
                    className={`w-5 h-5 ${isLoadingSuggestions ? "animate-spin" : ""} ${darkMode ? "text-blue-400" : "text-blue-600"}`}
                  />
                </motion.button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  <div
                    className={`w-16 text-center px-2 py-1 rounded text-xs ${
                      difficulty < 0.3
                        ? darkMode
