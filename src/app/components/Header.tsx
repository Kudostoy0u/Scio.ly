...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const isIosDevice =
      ...
      ((navigator as Navigator & { platform?: string; maxTouchPoints?: number }).platform === "MacIntel" && 
       (navigator as Navigator & { maxTouchPoints?: number }).maxTouchPoints && 
       (navigator as Navigator & { maxTouchPoints?: number }).maxTouchPoints! > 1);

    const isChromiumBrand = !!uaData?.brands?.some((b: any) =>
      /Chromium|Google Chrome|Microsoft Edge|Opera|OPR|Brave|Vivaldi|YaBrowser|Samsung Internet/i.test(
        b.brand
      )
    );
    ...
    const isSafariEngine =
      /safari/.test(ua) && !/chrome|crios|crmo|edg|opr|firefox|fxios|brave|electron/.test(ua);

    setIsSafari(Boolean(isSafariEngine || isIosDevice));
    ...
    const beforeInstall = (event: Event) => {
      ...
      try {
        (window as Window & { __deferredInstallPrompt__?: Event }).__deferredInstallPrompt__ = event;
      } catch {}
    };
    const onInstalled = () => {
      ...
      try {
        (window as Window & { __deferredInstallPrompt__?: Event }).__deferredInstallPrompt__ = undefined;
      } catch {}
    };
  ...
  useEffect(() => {
    const deferred = typeof window !== "undefined" && (window as Window & { __deferredInstallPrompt__?: Event }).__deferredInstallPrompt__;
  ...
  const computedOpacity = isTestPage
    ? headerOpacity
    : isHomePage
      ? Math.min(scrollOpacity * targetOpacity, targetOpacity)
      : targetOpacity;
              ...
              {isSafari ? (
                  ...
                  {canPromptInstall ? (
                    <ol className="list-decimal pl-5 space-y-1">
                      ...
                      <li className="flex items-start gap-2">
                        ...
                        <span>
                          Then choose &quot;Install&quot; in the browser dialog to add Scio.ly to
                          your home screen.
                        </span>
                      </li>
                    </ol>
                    ...
                    <ol className="list-decimal pl-5 space-y-2 pb-3">
                      <li className="flex items-start gap-3 pb-3">
                        ...
                        <span>
                          Install prompt not available. Your browser may not support in-app prompts
                ...
                <div>
                  ...
                  <div className={`${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    Firefox does not support installing this PWA. Please use Safari (iOS/macOS) or a
                    Chromium browser (Chrome/Brave/Edge) to install the app.
                  </div>
                </div>
              ) : (
                <div>
                  ...
                  <div className={`${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    Your browser may not support in-app install prompts. Try your browser’s menu and
