  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <div className="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs" style={{ display: photoUrl || user.user_metadata?.avatar_url || user.user_metadata?.picture ? 'none' : 'flex' }}>
          ...
          <div className={`px-4 py-2 text-sm ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {unreadCount > 0 && <span className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-red-600 text-white text-[10px]">{unreadCount > 9 ? '9+' : unreadCount}</span>}
                <div className="flex items-center gap-2">
                  {notifications.length > 0 && (
                    <button disabled={clearingAll} onClick={async () => { setClearingAll(true); await markAllAsRead(); setClearingAll(false); }} className={`${darkMode ? 'text-gray-300 hover:text-gray-100' : 'text-gray-600 hover:text-gray-800'} text-[10px] underline`} title="Clear all notifications">
                  ...
                  <div key={n.id} className={`rounded border ${darkMode ? 'border-gray-700' : 'border-gray-200'} p-2 text-xs`}>
                    ...
                    {n.type === 'assignment' ? (
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <button onClick={async () => { 
                          try { 
                            console.log('UserDropdown: Accepting assignment invitation:', n.id);
                            ...
                            console.error('Error accepting assignment invitation:', error);
                          }
                        ...
                        <button onClick={async () => { 
                          try {
                            console.log('UserDropdown: Declining assignment invitation:', n.id);
                            // For assignment invitations, we still need to use the API since it involves complex team logic
                            ...
                            console.error('Error declining assignment invitation:', error);
                          }
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <button onClick={async () => { 
                          try { 
                            console.log('UserDropdown: Accepting roster link invitation:', n.id);
                            ...
                            console.log('Accepting invitation with data:', requestBody);
                            const res = await fetch('/api/notifications/accept', { 
                              method: 'POST', 
                              headers: { 'Content-Type': 'application/json' }, 
                            ...
                            const j = await res.json(); 
                            console.log('Accept response data:', j);
                            if (res.ok && j?.success) { 
                              await markAsRead(n.id); 
                              await refresh(true);
                              // Invalidate teams cache to ensure the new team shows up
                        ...
                        <button onClick={async () => { 
                          try {
                            console.log('UserDropdown: Declining roster link invitation:', n.id);
                            // For roster link invitations, we still need to use the API since it involves complex team logic
                            const res = await fetch('/api/notifications/decline', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
