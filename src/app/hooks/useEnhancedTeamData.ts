...
export function useEnhancedTeamData() {
  ...
  const fetchRoster = useCallback(async (teamSlug: string, subteamId: string): Promise<{
    roster: Record<string, string[]>;
    removedEvents: string[];
  }> => {
    if (!teamSlug || !subteamId) return { roster: {}, removedEvents: [] };
  ...
  // Fetch stream data
  const fetchStream = useCallback(async (teamSlug: string, subteamId: string): Promise<any[]> => {
  ...
  // Fetch assignments
  const fetchAssignments = useCallback(async (teamSlug: string): Promise<any[]> => {
    if (!teamSlug) return [];
  ...
  // Fetch tournaments
  const fetchTournaments = useCallback(async (teamSlug: string, subteamId: string): Promise<any[]> => {
    if (!teamSlug || !subteamId) return [];
  ...
  // Fetch timers
  const fetchTimers = useCallback(async (teamSlug: string, subteamId: string): Promise<any[]> => {
    if (!teamSlug || !subteamId) return [];
  ...
  const loadRoster = useCallback(async (teamSlug: string, subteamId: string) => {
    try {
      ...
      const cacheKey = `roster-${teamSlug}-${subteamId}`;
      globalApiCache.startBackgroundRefresh(cacheKey, () => fetchRoster(teamSlug, subteamId), 'roster');
    } catch (err) {
  ...
  const loadMembers = useCallback(async (teamSlug: string, subteamId?: string) => {
    try {
      ...
      const cacheKey = `members-${teamSlug}-${subteamId || 'all'}`;
      globalApiCache.startBackgroundRefresh(cacheKey, () => fetchMembers(teamSlug, subteamId), 'members');
    } catch (err) {
  ...
  const loadStream = useCallback(async (teamSlug: string, subteamId: string) => {
    try {
      ...
      const cacheKey = `stream-${teamSlug}-${subteamId}`;
      globalApiCache.startBackgroundRefresh(cacheKey, () => fetchStream(teamSlug, subteamId), 'stream');
    } catch (err) {
  ...
  const loadAssignments = useCallback(async (teamSlug: string) => {
    try {
      ...
      const cacheKey = `assignments-${teamSlug}`;
      globalApiCache.startBackgroundRefresh(cacheKey, () => fetchAssignments(teamSlug), 'assignments');
    } catch (err) {
  ...
  const loadTournaments = useCallback(async (teamSlug: string, subteamId: string) => {
    try {
      ...
      const cacheKey = `tournaments-${teamSlug}-${subteamId}`;
      globalApiCache.startBackgroundRefresh(cacheKey, () => fetchTournaments(teamSlug, subteamId), 'tournaments');
    } catch (err) {
  ...
  const loadTimers = useCallback(async (teamSlug: string, subteamId: string) => {
    try {
      ...
      const cacheKey = `timers-${teamSlug}-${subteamId}`;
      globalApiCache.startBackgroundRefresh(cacheKey, () => fetchTimers(teamSlug, subteamId), 'timers');
    } catch (err) {
