...
export default function UnlimitedPracticePage({
	...
	useEffect(() => {
		...
		const routerParams =
			initialRouterData || (storedParams ? JSON.parse(storedParams) : {});
			...
			if (answeredInBatch + 1 >= batchSize) {
				...
				(async () => {
					...
					try {
						...
						const routerParams =
							initialRouterData || (paramsStr ? JSON.parse(paramsStr) : {});
						let questions = await fetchQuestionsForParams(
							routerParams,
							batchSize,
						);
						const seed = Date.now();
					...
					>
						{isLoading ? (
							...
							<div className="flex flex-col items-center justify-center h-64">
								...
								>
									Please select &quot;MCQ + FRQ&quot; in the dashboard to
									practice this event
								</p>
							</div>
						) : currentQuestion ? (
