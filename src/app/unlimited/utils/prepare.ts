...
export function prepareUnlimitedQuestions(input: PrepareInput): PrepareResult {
  ...
  const pctRaw = typeof idPercentage === 'string' ? parseInt(idPercentage) : (idPercentage ?? 0);
  const pct = Math.max(0, Math.min(100, Number.isFinite(pctRaw as number) ? (pctRaw as number) : 0));
  ...
  if (pct === 100) {
    finalQuestions = Array.from({ length: idCount }, (_, i) => ({
  ...
  finalQuestions.forEach((q, idx) => {
    if ((q as any)._isIdPlaceholder) idIndices.push(idx);
  });
