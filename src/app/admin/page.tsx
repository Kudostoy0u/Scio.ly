    ...
    <ul className="list-disc ml-5">
      {options.map((o, i) => (
        <li
          ...
          className={
            answers.includes(i) || answers.includes(o) ? "text-green-600 dark:text-green-400" : ""
          }
        >
          {String(o)}
        </li>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <button type="button" onClick={onApplyAll} disabled={true} className="hidden" />
        <button
          type="button"
          onClick={onUndoAll}
...
export default function AdminPage() {
  ...
  const updateDataAfterUndo = useCallback((id: string, action: "undoEdit" | "undoRemove") => {
    if (action === "undoEdit") {
      setData((prev) => ({
        ...prev,
