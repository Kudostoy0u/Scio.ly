...
export default function ReportsPage() {
	...
	const [blacklistedQuestions, setBlacklistedQuestions] = useState<
		Record<string, string[]>
	>({});
	const [editedQuestions, setEditedQuestions] = useState<
		Record<
			string,
			Array<{ original: string; edited: string; timestamp: string }>
		>
	>({});
	...
	const [activeTab, setActiveTab] = useState<"blacklisted" | "edited">(
		"blacklisted",
	);
		...
		const load = async () => {
			...
			try {
				...
				if (
					...
					blacklistedQuestions[selectedEvent] === undefined
				) {
					const res = await fetch(
						`${api.blacklists}?event=${encodeURIComponent(selectedEvent)}`,
					);
					if (!res.ok) {
				...
				if (
					activeTab === "edited" &&
					editedQuestions[selectedEvent] === undefined
				) {
					const res = await fetch(
						`${api.edits}?event=${encodeURIComponent(selectedEvent)}`,
					);
					if (!res.ok) {
	...
	const getEventsWithReports = () => {
		const eventsWithReports = new Set<string>([
			...
			...Object.keys(removedByEvent).filter(
				(e) => (removedByEvent[e] || 0) > 0,
			),
		]);
	...
	return (
		<div className={`min-h-screen ${bgColor} ${textColor}`}>
			<div className="container mx-auto px-4 py-12">
					...
					<div className="flex flex-col md:flex-row items-start md:items-center justify-between">
						<div>
							...
							<p className={`${secondaryTextColor} max-w-2xl mb-3`}>
								Thanks to all of our users for helping us maintain and improve
								our question bank. Your reports make Science Olympiad practice
								better for everyone!
							</p>
							<div
							...
							>
								<div className="flex">
									...
									<p className="text-sm font-medium">
										Please keep reports concise and clear! This helps our team
				...
				{/* Event Selection */}
				<div
					className={`${cardBgColor} rounded-lg shadow-md border ${borderColor} p-6 mb-8`}
				>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
								{getEventsWithReports().map((event) => (
									...
									>
										...
										<div className={`text-xs ${mutedTextColor} mb-1`}>
											{event.subject}
										</div>
					...
					<div className="p-6">
						{loading ? (
								...
								<div className="flex items-center justify-between mb-6">
									<div>
										...
										<p className={`${mutedTextColor} mt-1`}>
											{activeTab === "blacklisted" ? "Blacklisted" : "Edited"}{" "}
											Questions
										</p>
									</div>
									<div
								...
								{getCurrentEventData().length === 0 ? (
									...
									>
										...
										<p className="text-lg">
											No{" "}
											{activeTab === "blacklisted" ? "blacklisted" : "edited"}{" "}
											questions found for {selectedEvent}
										</p>
							...
							>
								...
								<p className="text-sm mt-2">
									Choose an event from the list above to see blacklisted or
									edited questions
								</p>
							</div>
						)}
				...
				>
					...
					<p className={secondaryTextColor}>
						Your reports help us maintain the highest quality question bank for
						Science Olympiad students. Together, we&apos;re building a better
