...
export const ScrollBarAlwaysVisible = ({
	children,
	darkMode,
	...
	const recalc = React.useCallback(() => {
		...
		const computedThumbHeight = Math.max(
			minThumb,
			Math.floor((trackHeight / total) * trackHeight),
		);
	...
	return (
		<div className="h-full relative">
			<div
				...
				className="h-full overflow-y-auto pr-2 native-scroll-hidden"
			>
				{children}
			</div>
			{isScrollable && (
				<div className="pointer-events-none absolute inset-y-0 right-0 w-1.5">
					<div
						className={`absolute inset-y-0 right-0 w-1.5 rounded-full ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
					/>
					<div
						ref={thumbRef}
						className={`absolute right-0 w-1.5 rounded-full will-change-transform ${darkMode ? "bg-gray-500" : "bg-gray-400"}`}
						style={{
