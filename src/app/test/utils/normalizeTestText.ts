...
export function normalizeTestText(input: string | null | undefined): string {
	...
	const pointPattern =
		/\s*[([]\s*\d+(?:\.\d+)?\s*(?:pt|pts|point|points)\s*[)\]]\s*/gi;
...
function parseLeadingLetterLabel(
...
function isSequentialFromA(labels: string[]): boolean {
	...
	for (let i = 0; i < labels.length; i++) {
		...
		if (code !== undefined && code !== "A".charCodeAt(0) + i) {
			return false;
		}
	}
	return true;
...
export function normalizeOptionAnswerLabels<T extends string | number>(
	options: (string | null | undefined)[] | null | undefined,
	answers: T[] | null | undefined,
): { options: string[]; answers: T[] } {
	try {
		if (!Array.isArray(options) || options.length === 0) {
			return {
				options:
					options?.filter((o): o is string => o !== null && o !== undefined) ||
					[],
				answers: answers || [],
			};
		}
