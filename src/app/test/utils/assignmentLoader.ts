...
export async function loadAssignment(
  ...
  try {
    ...
    if (assignmentId && loadFromNewFormat(assignmentId, stableRouterData, callbacks)) {
      return;
    }
...
export function loadViewResultsData(
  ...
  const { setData, setUserAnswers, setGradingResults, setIsLoading, fetchCompletedRef } = callbacks;
