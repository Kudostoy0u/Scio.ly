...
export async function fetchBaseQuestions(routerParams: RouterParams, count: number): Promise<Question[]> {
  ...
  if (isOffline) {
    ...
    if (!apiResponse) throw new Error('No offline data available for this event.');
  } else {
...
export async function fetchIdQuestions(routerParams: RouterParams, idCount: number): Promise<Question[]> {
  if (idCount <= 0) return [];
  const isOffline = typeof navigator !== 'undefined' ? !navigator.onLine : false;
  let source: any[] = [];
  if (!isOffline) {
    try {
      ...
      if (routerParams.types) {
        ...
        else if (routerParams.types === 'free-response') params.set('question_type', 'frq');
      }
