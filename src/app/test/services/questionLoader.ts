...
} from "@/app/test/utils/questionMedia";
...
export async function fetchIdQuestions(
	...
	if (!isOffline) {
		try {
			...
			const subtopics = Array.isArray(routerParams.subtopics)
				? routerParams.subtopics
				: [];
			...
			if (
				...
				(routerParams.rmTypeFilter === "rock" ||
					routerParams.rmTypeFilter === "mineral")
			) {
		...
		return {
			...
			imageData: buildAbsoluteUrl(
				Array.isArray(rowRecord.images) && rowRecord.images.length > 0
					? String(
							rowRecord.images[
								Math.floor(Math.random() * rowRecord.images.length)
							],
						)
					: undefined,
			),
