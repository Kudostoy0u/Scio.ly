...
import * as storage_1 from "@/app/utils/storage";
import * as questionUtils_1 from "@/app/utils/questionUtils";
import * as questionMedia_1 from "../utils/questionMedia";
import * as normalizeTestText_1 from "../utils/normalizeTestText";
...
async function fetchBaseQuestions(routerParams, count) {
    ...
    const params = (0, questionUtils_1.buildApiParams)(paramsObj, count);
    ...
    const filtered = typesSel === 'multiple-choice'
        ...
        : typesSel === 'free-response'
            ? allQuestions.filter((q) => !Array.isArray(q.options) || q.options.length === 0)
            : allQuestions;
    return (0, questionMedia_1.normalizeQuestionMedia)(filtered);
}
async function fetchIdQuestions(routerParams, idCount) {
    ...
    if (!isOffline) {
        try {
            ...
            if (routerParams.difficulties && routerParams.difficulties.length > 0) {
                const allRanges = routerParams.difficulties.map((d) => questionUtils_1.difficultyRanges[d]).filter(Boolean);
            ...
            if (routerParams.pureIdOnly === true || routerParams.pureIdOnly === 'true' || routerParams.pureIdOnly === '1' || routerParams.pureIdOnly === 1) {
                params.set('pure_id_only', 'true');
            }
            const resp = await fetch(`${api_1.default.idQuestions}?${params.toString()}`);
            const json = await resp.json();
            source = Array.isArray(json === null || json === void 0 ? void 0 : json.data) ? json.data : [];
        }
        catch { }
    }
    const idQuestions = source.map((row) => ({
        ...
        imageData: (0, questionMedia_1.buildAbsoluteUrl)(Array.isArray(row.images) && row.images.length ? row.images[Math.floor(Math.random() * row.images.length)] : undefined),
    }));
    return idQuestions;
}
...
function finalizeQuestions(questions) {
    const finalized = (0, questionMedia_1.normalizeQuestionMedia)(questions);
    finalized.forEach((question) => {
        if (question.question) {
            question.question = (0, normalizeTestText_1.normalizeQuestionText)(question.question);
        }
        if (Array.isArray(question.options)) {
            question.options = question.options.map((option) => typeof option === 'string' ? (0, normalizeTestText_1.normalizeTestText)(option) : option);
            const normalized = (0, normalizeTestText_1.normalizeOptionAnswerLabels)(question.options, Array.isArray(question.answers) ? question.answers : []);
        ...
        if (Array.isArray(question.answers)) {
            question.answers = question.answers.map((answer) => typeof answer === 'string' ? (0, normalizeTestText_1.normalizeTestText)(answer) : answer);
        }
    });
    return finalized;
}
