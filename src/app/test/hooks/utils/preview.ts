...
function extractAnswerPicks(question: Question): string[] {
  const answerList = Array.isArray(question.answers) ? question.answers : [question.answers];
  const picks: string[] = [];
...
function processFreeResponseQuestion(
  ...
  const corrects = Array.isArray(question.answers) ? question.answers : [question.answers];
