...
describe('normalizeQuestionsFull', () => {
  ...
  describe('Answer Field Preservation', () => {
    it('should preserve numeric answers for MCQ questions', () => {
      ...
      const result = normalizeQuestionsFull(questions);
  ...
  describe('Assignment Questions', () => {
    ...
    it('should handle complex assignment question structure', () => {
      const questions = [
        {
          ...
          question: 'Centromeres of sister chromatids uncouple and separate during which phase?',
          options: ['Metaphase I', 'Metaphase II', 'Anaphase I', 'Anaphase II'],
          answers: [3],
          difficulty: 0.4,
          division: 'C',
      ...
      expect(result).toHaveLength(1);
      expect(result[0].answers).toEqual([3]);
      expect(result[0].question).toBe('Centromeres of sister chromatids uncouple and separate during which phase?');
      expect(result[0].options).toEqual(['Metaphase I', 'Metaphase II', 'Anaphase I', 'Anaphase II']);
    });
  });
});
