...
export function fuzzyGrade(student: string, corrects: (string | number)[]): number {
...
export async function gradeFrqBatch(frqsToGrade: FrqItem[], online: boolean): Promise<number[]> {
  const { gradeFreeResponses } = await import("@/app/utils/questionUtils");
  if (!online) {
    return frqsToGrade.map((item) => fuzzyGrade(item.studentAnswer, item.correctAnswers));
  }
  try {
