...
exports.useTestGrading = useTestGrading;
...
function correctInvalidScores(invalid, data, userAnswers, calculateMCQScore) {
	const corrected = {};
...
async function computeMissingGrades(
	...
	try {
		const { gradeMissing } = await Promise.resolve().then(() =>
			require("./utils/grading"),
		);
		const computed = gradeMissing(
			data,
			userAnswers,
			calculateMCQScore,
			missing,
		...
		if (Object.keys(computed).length > 0) {
			setGradingResults((prev) => __assign(__assign({}, prev), computed));
		}
	} catch (error) {
		logger_1.default.error("Error computing missing grades:", error);
	}
}
/**
 ...
 * ```
 */
function useTestGrading(data, userAnswers, isSubmitted, routerData) {
	// State for grading results - maps question index to score (0 = wrong, 0.5 = partial, 1 = correct)
	const [gradingResults, setGradingResults] = (0, react_1.useState)({});
	// State for FRQ grading loading indicators - maps question index to loading state
	const [gradingFrQs, setGradingFrQs] = (0, react_1.useState)({});
	/**
	...
	(0, react_1.useEffect)(() => {
		...
		const { missing, invalid } = validateGradingResults(
			...
			userAnswers,
			gradingResults,
			questionUtils_1.calculateMCQScore,
		);
		// Pass 2: Fix invalid scores immediately (synchronous)
		if (invalid.length > 0) {
			const corrected = correctInvalidScores(
				invalid,
				data,
				userAnswers,
				questionUtils_1.calculateMCQScore,
			);
			setGradingResults((prev) => __assign(__assign({}, prev), corrected));
		}
	...
	(0, react_1.useEffect)(() => {
		try {
			...
			logger_1.default.error(
				"Error persisting grading results to localStorage:",
				error,
			);
		}
