...
function persistGradingResults(
	validatedGrading: GradingResults,
...
export function useTestSubmission(
	...
	const handleSubmit = useCallback(async () => {
		...
		await submitEnhancedAssignment(
			routerData,
			data,
			userAnswers,
			mcqScore,
	...
	const reloadQuestions = useCallback(async () => {
		...
		try {
			const total = Number.parseInt(
				(routerData.questionCount as string) || "10",
			);
