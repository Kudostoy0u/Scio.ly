...
const EventStatistics = ({
	...
	const unlimitedCount = questions.filter(
		(q) => q.source === "unlimited",
	).length;
...
export default function Content() {
	...
	const handleTakeTimedTest = (
		eventName: string,
		_questions: BookmarkedQuestion[],
	) => {
		(async () => {
			if (!user) {
	...
	const handleRemoveSingleBookmark = async (
		questionToRemove: BookmarkedQuestion,
	) => {
		if (!user) {
			toast.info("Please sign in to manage bookmarks");
		...
		try {
			await removeBookmark(
				...
				questionToRemove.source,
			);
			setBookmarkedQuestions((prev) =>
				updateBookmarksAfterRemoval(prev, questionToRemove),
			);
			...
			<div className="relative z-10 pt-20 px-4 sm:px-6">
				<div className="max-w-7xl mx-auto">
					...
					<div className="mb-8">
						...
						>
							Here you can find all the questions you&apos;ve bookmarked while
							practicing or taking tests, organized by event. Click the arrow on
							any event card to start a practice session with those specific
							questions.
						</p>
					</div>
