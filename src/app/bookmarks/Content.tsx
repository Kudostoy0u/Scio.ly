  ...
  return (
    ...
    >
      ...
      {(!question.options || question.options.length === 0) && question.answers.length > 0 && (
        <div className="mt-2">
          <h4 className={`text-xs font-semibold mb-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
            Correct Answer:
          </h4>
          <div
  ...
  const loadBookmarks = async (userId: string) => {
    ...
    try {
      ...
      if (bookmarks && bookmarks.length > 0) {
        const groupedQuestions = bookmarks.reduce(
          (acc, question) => {
                          ...
                          >
                            <div className="flex justify-between items-center">
                              <div>
                                ...
                                <div
                                  className={`hidden md:flex gap-4 mt-1 ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                                >
                                  <p>
                                  ...
                                  <p>
                                    From Test Mode:{" "}
                                    {questions.filter((q) => q.source === "test").length}
                                  </p>
                                  <p>
                                    From Unlimited Mode:{" "}
                                    {questions.filter((q) => q.source === "unlimited").length}
                                  </p>
                                </div>
                                {/* Simple count for mobile only */}
                                <div
                                  className={`md:hidden mt-1 ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                                >
                                  <p>
                                    {questions.length} question{questions.length !== 1 ? "s" : ""}
                                  </p>
                              ...
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className={`h-5 w-5 transition-transform duration-300 ${openEvents[eventName] ? "rotate-180" : ""} ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                              >
