...
export default function DifficultyDropdown({
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className={`absolute z-10 mt-1 w-full rounded-md shadow-lg ${darkMode ? 'bg-gray-700' : 'bg-white'} ring-1 ring-black ring-opacity-5`}>
          <div className="py-1">
            {isCodebusters
              ? codebustersDifficulties.map((difficulty) => (
                  <label key={difficulty} className={`flex items-center px-4 py-2 text-xs cursor-pointer ${darkMode ? 'text-gray-300 hover:bg-gray-600' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <input
                      type="checkbox"
              ...
              : standardDifficulties.map((difficulty) => (
                  <label key={difficulty.id} className={`flex items-center px-4 py-2 text-xs cursor-pointer ${darkMode ? 'text-gray-300 hover:bg-gray-600' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <input
                      type="checkbox"
