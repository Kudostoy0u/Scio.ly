...
import type { Settings, Event } from '../types';
...
export default function SubtopicDropdown({
  ...
  const renderCodebusters = () => {
    ...
    return groups.map((g) => (
      <div key={g.label} className="px-2 py-1">
        <div className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} text-[10px] uppercase tracking-wider mb-1`}>{g.label}</div>
        {g.items.map((cipherType) => (
          <label key={cipherType} className={`flex items-center px-4 py-2 text-xs cursor-pointer ${darkMode ? 'text-gray-300 hover:bg-gray-600' : 'text-gray-700 hover:bg-gray-100'}`}>
            <input
              type="checkbox"
  ...
  const renderSubtopics = () => {
    const topics = selectedEvent ? (window as any).eventSubtopicsMapping?.[selectedEvent.name] : null;
    if (!topics) return null;
    return topics.map((subtopic: string) => (
      <label key={subtopic} className={`flex items-center px-4 py-2 text-xs cursor-pointer ${darkMode ? 'text-gray-300 hover:bg-gray-600' : 'text-gray-700 hover:bg-gray-100'}`}>
        <input
          type="checkbox"
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className={`absolute z-10 mt-1 w-full rounded-md shadow-lg ${darkMode ? 'bg-gray-700' : 'bg-white'} ring-1 ring-black ring-opacity-5 max-h-60 overflow-y-auto`}>
          <div className="py-1">
            {isCodebusters ? renderCodebusters() : renderSubtopics()}
          </div>
        </div>
      )}
    </div>
