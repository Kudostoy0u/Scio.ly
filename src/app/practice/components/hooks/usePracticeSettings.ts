...
export function usePracticeSettings() {
	...
	useEffect(() => {
		if (typeof window !== "undefined") {
			const storedQuestionCount = SyncLocalStorage.getItem(
				"defaultQuestionCount",
			);
			const storedTimeLimit = SyncLocalStorage.getItem("defaultTimeLimit");
			const storedDivision =
				SyncLocalStorage.getItem("defaultDivision") || "any";
			const storedQuestionTypes =
				SyncLocalStorage.getItem("defaultQuestionTypes") || "multiple-choice";
			const storedIdPercentage = SyncLocalStorage.getItem(
				"defaultIdPercentage",
			);
			const storedPureIdOnly = SyncLocalStorage.getItem("defaultPureIdOnly");
			const storedCharLengthMin = SyncLocalStorage.getItem(
				"codebustersCharLengthMin",
			);
			const storedCharLengthMax = SyncLocalStorage.getItem(
				"codebustersCharLengthMax",
			);
			...
			const idPercentage = storedIdPercentage
				? Number.parseInt(storedIdPercentage)
				: 0;
			const pureIdOnly = storedPureIdOnly === "true";
			const charLengthMin = storedCharLengthMin
				? Number.parseInt(storedCharLengthMin)
				: 1;
			const charLengthMax = storedCharLengthMax
				? Number.parseInt(storedCharLengthMax)
				: 100;

			setSettings((prev: Settings) => ({
				...prev,
				questionCount: Number.isNaN(questionCount)
					? NORMAL_DEFAULTS.questionCount
					: questionCount,
				timeLimit: Number.isNaN(timeLimit)
					? NORMAL_DEFAULTS.timeLimit
					: timeLimit,
				division:
					storedDivision === "B" ||
