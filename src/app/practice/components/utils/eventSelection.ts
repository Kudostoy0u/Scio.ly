...
export function supportsPictureQuestions(eventName: string): boolean {
	...
	return (
		PICTURE_QUESTION_EVENTS.includes(eventName) ||
		PICTURE_QUESTION_EVENTS.includes(base)
	);
}
...
function getCodebustersSettings(event: Event): Partial<Settings> {
	if (typeof window !== "undefined") {
		const codebustersQuestionCount = SyncLocalStorage.getItem(
			"codebustersQuestionCount",
		);
		const codebustersTimeLimit = SyncLocalStorage.getItem(
			"codebustersTimeLimit",
		);

		const questionCount = codebustersQuestionCount
			? Number.parseInt(codebustersQuestionCount)
			: 3;
		const timeLimit = codebustersTimeLimit
			? Number.parseInt(codebustersTimeLimit)
			: 15;
		...
		const divisionForEvent = getDivisionForEvent(
			validDivision,
			availableDivisions,
		);
...
export function getSettingsForEvent(event: Event): Partial<Settings> {
	...
	const defaultTimeLimit = getStoredNumber(
		"defaultTimeLimit",
	...
	const validTypes =
		savedTypes === "multiple-choice" ||
		savedTypes === "both" ||
		savedTypes === "free-response"
			? savedTypes
			: "multiple-choice";
	...
	const savedIdPercentage = supportsPicture
		? getStoredNumber("defaultIdPercentage", 0)
		: 0;
	const savedPureIdOnly =
		supportsIdOnly && getStoredValue("defaultPureIdOnly", "false") === "true";
