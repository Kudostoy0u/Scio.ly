...
export function computeContinueInfo(): ContinueInfo {
  ...
  try {
    ...
    const session = sessionStr
      ? (JSON.parse(sessionStr) as { eventName?: string; isSubmitted?: boolean })
      : null;
    ...
    const hasGeneralProgress =
      ...
      Object.values(testAnswers).some((arr) =>
        Array.isArray(arr) ? arr.some((v) => v && String(v).length > 0) : !!testAnswersStr
      );
      ...
      try {
        ...
        hasCodebustersProgress =
          ...
          quotes.some((q) => {
            const hasSolution = !!(
              q?.solution &&
              Object.values(q.solution).some((v) => typeof v === "string" && v.length > 0)
            );
            const hasHill = !!(
            ...
            const hasNihilist = !!(
              q?.nihilistSolution &&
              Object.values(q.nihilistSolution).some((v) => typeof v === "string" && v.length > 0)
            );
            ...
            const hasColumnar = !!(
              q?.columnarSolution &&
              Object.values(q.columnarSolution).some((v) => typeof v === "string" && v.length > 0)
            );
            const hasXeno = !!(
              q?.xenocryptSolution &&
              Object.values(q.xenocryptSolution).some((v) => typeof v === "string" && v.length > 0)
            );
            const hasNotes = !!(
              q?.frequencyNotes &&
              Object.values(q.frequencyNotes).some((v) => typeof v === "string" && v.length > 0)
            );
            return (
              hasSolution ||
              hasHill ||
