...
import {
	processFrqExplanation,
	processMcqExplanation,
} from "./explanationLogic";
...
export const buildApiParams = (
	...
	const { eventName, difficulties, division, tournament, subtopics, types } =
		routerParams;
	...
	try {
		...
		if (question.imageUrl || question.imageData) {
			...
			if (imageUrls.length > 0) {
				requestBody.imageUrls = imageUrls.filter(
		...
		if (!response.ok) {
			...
			throw new Error(
				`Failed to fetch explanation: ${response.status} ${response.statusText}`,
			);
		}
...
const handleExplanationResult = async (
	...
	if (result.shouldSubmitEdit && result.updatedQuestion) {
		logger.log(
			"✅ Explanation suggested different answers, submitting edit request.",
		);
...
export const calculateMCQScore = (
	...
	const correctOptions = question.answers.map(
		(ans) => question.options?.[Number(ans)],
	);
	...
	if (isMultiSelectQuestion(question.question, question.answers)) {
		...
		const numCorrectSelected = filteredUserAnswers.filter((a) =>
			correctOptions.includes(a),
		).length;
		const hasIncorrectAnswers = filteredUserAnswers.some(
			(a) => !correctOptions.includes(a),
		);
	...
	const result =
		filteredUserAnswers.length === 1 &&
		filteredUserAnswers[0] === correctOptions[0]
			? 1
			: 0;
	return result;
};
