...
export const handleShareCodeRedirect = async (code: string): Promise<boolean> => {
  ...
  const timeLimit = Number.parseInt((result.testParams.timeLimit as string) || "30");
  const isSharedTest = true;
  const baseRemaining =
    typeof result.adjustedTimeRemaining === "number" ? result.adjustedTimeRemaining : null;
  ...
  if (result.eventName === "Codebusters") {
    ...
    if (result.questions && Array.isArray(result.questions)) {
      const questionsWithIndex = result.questions.map(
        (q: Record<string, unknown>, idx: number) => ({ ...q, originalIndex: idx })
      );
