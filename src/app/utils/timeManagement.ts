...
export const resumeTestSession = (): TestSession | null => {
	...
	if (
		!timeState.isTimeSynchronized &&
		timeState.isPaused &&
		timeState.lastPauseTime
	) {
	...
	if (
		timeState.isTimeSynchronized &&
		timeState.syncTimestamp &&
		timeState.originalTimeAtSync
	) {
		const elapsedMs = now - timeState.syncTimestamp;
		const elapsedSeconds = Math.floor(elapsedMs / 1000);
		timeState.timeLeft = Math.max(
			0,
			timeState.originalTimeAtSync - elapsedSeconds,
		);
	} else {
...
export const migrateFromLegacyStorage = (
	...
	const isTimeSynchronized =
		SyncLocalStorage.getItem("isTimeSynchronized") === "true";
	...
	if (legacyTimeLeft || legacyCodebustersTimeLeft) {
		const timeLeft =
			legacyTimeLeft || legacyCodebustersTimeLeft || timeLimit * 60;
		const now = Date.now();
