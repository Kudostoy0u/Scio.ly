	...
	const allSeasons = [
		...new Set(
			events.flatMap((event) =>
				Object.keys(
					(data[firstSchool] as Record<string, Record<string, number>>)?.[
						event
					] || {},
				),
			),
		...
		data: {
			...
			datasets: schools.flatMap((school, schoolIndex) =>
				events.map((event, eventIndex) => ({
					...
					data: seasons.map(
						(season) =>
							(data[school] as Record<string, Record<string, number>>)?.[
								event
							]?.[season] || null,
					),
					borderColor:
						CHART_COLORS[
							(schoolIndex * events.length + eventIndex) % CHART_COLORS.length
						] ?? "#000000",
