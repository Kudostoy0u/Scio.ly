...
export default function JsonDataPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(api.reportAll);
        if (!response.ok) {
  ...
  const handleSetScore = async () => {
    updateGamePoints(currentUser?.id || null, parseInt(adminNewScore,10))
        ...
        <div className={`mt-8 p-6 rounded-lg shadow-md border ${borderColor} ${cardBgColor}`}>
            ...
            {currentUser ? (
               <div className="space-y-4">
                  ...
                  */}
                 <div>
                                           <label htmlFor="adminNewScore" className="block text-sm font-medium mb-1">New Game Score for {currentUser.email || currentUser.id}:</label>
                     <input
                         type="number"
                         id="adminNewScore"
                         value={adminNewScore}
                 ...
                 <button
                     ...
                     className={`px-5 py-2 rounded-md font-semibold  flex items-center justify-center gap-2 ${isSubmittingAdmin ? 'bg-gray-500 cursor-not-allowed' : (darkMode ? 'bg-blue-600 hover:bg-blue-500' : 'bg-blue-500 hover:bg-blue-600')} text-white`}
                 >
                     {isSubmittingAdmin ? (
