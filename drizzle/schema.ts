import { cockroachSchema, cockroachTable, uuid, int8, varchar, timestamp, string, char, jsonb, decimal, bool, float, time, date, index, uniqueIndex, foreignKey, primaryKey, check, cockroachView, customType, text } from "drizzle-orm/cockroach-core"
...
export const apiKeyGenerations = cockroachTable("api_key_generations", {
	id: uuid().defaultRandom().primaryKey(),
	ipAddress: string("ip_address").notNull(),
	apiKeyHash: string("api_key_hash").notNull(),
	userId: uuid("user_id"),
	generatedAt: timestamp("generated_at", { mode: 'string', withTimezone: true }).defaultNow(),
	...
	uniqueIndex("idx_api_key_generations_ip_unique").using("btree", table.ipAddress.asc()),
...
export const assignments = cockroachTable("assignments", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	...
	index("idx_assignments_team").using("btree", table.school.asc(), table.division.asc(), table.teamId.asc()),
check("assignments_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);

export const base52Codes = cockroachTable("base52_codes", {
	id: uuid().defaultRandom().primaryKey(),
	...
	index("base52_codes_question_table_idx").using("btree", table.questionId.asc(), table.tableName.asc()),
	uniqueIndex("base52_codes_unique_code").using("btree", table.code.asc()),
	uniqueIndex("base52_codes_unique_question").using("btree", table.questionId.asc(), table.tableName.asc()),
check("check_table_name", sql`CHECK ((table_name IN ('questions'::STRING, 'idEvents'::STRING)))`),]);

export const blacklists = cockroachTable("blacklists", {
	id: uuid().defaultRandom().primaryKey(),
	event: varchar({ length: 255 }).notNull(),
	questionData: jsonb("question_data").notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
}, (table) => [
	index("idx_blacklists_event").using("btree", table.event.asc()),
]);

export const edits = cockroachTable("edits", {
	id: uuid().defaultRandom().primaryKey(),
	event: varchar({ length: 255 }).notNull(),
	...
	index("idx_edits_event").using("btree", table.event.asc()),
]);

export const geminiExplanationsCache = cockroachTable("gemini_explanations_cache", {
	id: uuid().defaultRandom().primaryKey(),
	...
	index("idx_gemini_cache_question_hash").using("btree", table.questionHash.asc(), table.event.asc(), table.userAnswer.asc()).where(sql`question_hash IS NOT NULL`),
	index("idx_gemini_cache_question_id").using("btree", table.questionId.asc(), table.event.asc(), table.userAnswer.asc()).where(sql`question_id IS NOT NULL`),
check("check_identifier", sql`CHECK (((question_id IS NOT NULL) OR (question_hash IS NOT NULL)))`),]);

export const idEvents = cockroachTable("id_events", {
	id: uuid().primaryKey(),
	...
	index("idx_id_events_question_type").using("btree", table.questionType.asc()),
]);
...
export const invitesV2 = cockroachTable("invites_v2", {
	...
	index("idx_invites_v2_invitee").using("btree", table.inviteeUsername.asc(), table.status.asc()),
	index("idx_invites_v2_team").using("btree", table.school.asc(), table.division.asc(), table.teamId.asc()),
	uniqueIndex("uq_pending_invite").using("btree", table.inviteeUsername.asc(), table.school.asc(), table.division.asc(), table.teamId.asc(), table.status.asc()),
check("invites_v2_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);

export const longquotes = cockroachTable("longquotes", {
	id: uuid().defaultRandom().primaryKey(),
	...
	index("idx_longquotes_language_char_length").using("btree", table.language.asc(), table.charLength.asc()),
...
export const newTeamActiveTimers = cockroachTable("new_team_active_timers", {
	...
	uniqueIndex("new_team_active_timers_team_unit_id_event_id_key").using("btree", table.teamUnitId.asc(), table.eventId.asc()),
]);
...
export const newTeamAnalytics = cockroachTable("new_team_analytics", {
	...
	recordedAt: timestamp("recorded_at", { mode: 'string', withTimezone: true }).defaultNow(),
	periodStart: timestamp("period_start", { mode: 'string', withTimezone: true }),
...
export const newTeamAssignmentAnalytics = cockroachTable("new_team_assignment_analytics", {
	...
	index("idx_assignment_analytics_assignment").using("btree", table.assignmentId.asc()),
	index("idx_assignment_analytics_student").using("btree", table.studentName.asc()),
]);
...
export const newTeamAssignmentQuestionResponses = cockroachTable("new_team_assignment_question_responses", {
	...
	index("idx_question_responses_question").using("btree", table.questionId.asc()),
	index("idx_question_responses_submission").using("btree", table.submissionId.asc()),
	uniqueIndex("new_team_assignment_question_responses_submission_id_question_id_key").using("btree", table.submissionId.asc(), table.questionId.asc()),
]);
...
export const newTeamAssignmentQuestions = cockroachTable.withRLS("new_team_assignment_questions", {
	...
	index("idx_assignment_questions_assignment_id").using("btree", table.assignmentId.asc()),
	index("idx_assignment_questions_order").using("btree", table.assignmentId.asc(), table.orderIndex.asc()),
check("check_question_type", sql`CHECK ((question_type IN ('multiple_choice'::STRING, 'free_response'::STRING, 'codebusters'::STRING)))`),]);
...
export const newTeamAssignmentRoster = cockroachTable("new_team_assignment_roster", {
	...
	index("idx_assignment_roster_student").using("btree", table.studentName.asc(), table.subteamId.asc()),
	index("idx_assignment_roster_user").using("btree", table.userId.asc()).where(sql`user_id IS NOT NULL`),
	uniqueIndex("new_team_assignment_roster_assignment_id_student_name_subteam_id_key").using("btree", table.assignmentId.asc(), table.studentName.asc(), table.subteamId.asc()),
]);
...
export const newTeamAssignmentSubmissions = cockroachTable("new_team_assignment_submissions", {
	...
	submittedAt: timestamp("submitted_at", { mode: 'string', withTimezone: true }).defaultNow(),
	grade: int8({ mode: 'number' }),
	feedback: string(),
	status: varchar({ length: 20 }).default("submitted"),
	attemptNumber: int8("attempt_number", { mode: 'number' }).default(1),
}, (table) => [
	uniqueIndex("new_team_assignment_submissions_assignment_id_user_id_attempt_number_key").using("btree", table.assignmentId.asc(), table.userId.asc(), table.attemptNumber.asc()),
check("new_team_assignment_submissions_check_status", sql`CHECK ((status IN ('draft'::STRING, 'submitted'::STRING, 'graded'::STRING, 'returned'::STRING)))`),]);
...
export const newTeamAssignmentTemplates = cockroachTable("new_team_assignment_templates", {
	...
	index("idx_assignment_templates_event").using("btree", table.eventName.asc()),
	index("idx_assignment_templates_team").using("btree", table.teamId.asc()),
check("new_team_assignment_templates_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);
...
export const newTeamAssignments = cockroachTable("new_team_assignments", {
	...
	index("idx_new_team_assignments_due_date").using("btree", table.dueDate.asc()),
	index("idx_new_team_assignments_team_id").using("btree", table.teamId.asc()),
check("check_assignment_type", sql`CHECK ((assignment_type IN ('task'::STRING, 'homework'::STRING, 'project'::STRING, 'study'::STRING, 'other'::STRING)))`),]);
...
export const newTeamEventAttendees = cockroachTable("new_team_event_attendees", {
	...
	uniqueIndex("new_team_event_attendees_event_id_user_id_key").using("btree", table.eventId.asc(), table.userId.asc()),
check("new_team_event_attendees_check_status", sql`CHECK ((status IN ('pending'::STRING, 'attending'::STRING, 'declined'::STRING, 'tentative'::STRING)))`),]);
...
export const newTeamEvents = cockroachTable("new_team_events", {
	...
	reminderMinutes: int8("reminder_minutes", { mode: 'bigint' }).array().default([15n,60n,1440n]),
	createdAt: timestamp("created_at", { mode: 'string', withTimezone: true }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string', withTimezone: true }).defaultNow(),
}, (table) => [
	index("idx_new_team_events_start_time").using("btree", table.startTime.asc()),
	index("idx_new_team_events_team_id").using("btree", table.teamId.asc()),
check("check_event_type", sql`CHECK ((event_type IN ('practice'::STRING, 'tournament'::STRING, 'meeting'::STRING, 'deadline'::STRING, 'other'::STRING)))`),]);

export const newTeamGroups = cockroachTable("new_team_groups", {
	id: uuid().defaultRandom().primaryKey(),
	school: varchar({ length: 255 }).notNull(),
	division: char().notNull(),
	...
	index("idx_new_team_groups_archived").using("btree", table.status.asc()).where(sql`status = 'archived'::STRING`),
	index("idx_new_team_groups_school_division").using("btree", table.school.asc(), table.division.asc()),
...
check("new_team_groups_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),check("new_team_groups_status_check", sql`CHECK ((status IN ('active'::STRING, 'archived'::STRING, 'deleted'::STRING)))`),]);
...
export const newTeamInvitations = cockroachTable("new_team_invitations", {
	...
	index("idx_new_team_invitations_code").using("btree", table.invitationCode.asc()),
	index("idx_new_team_invitations_email").using("btree", table.email.asc()),
	uniqueIndex("new_team_invitations_invitation_code_key").using("btree", table.invitationCode.asc()),
check("check_status", sql`CHECK ((status IN ('pending'::STRING, 'accepted'::STRING, 'declined'::STRING, 'expired'::STRING)))`),check("new_team_invitations_check_role", sql`CHECK (("role" IN ('captain'::STRING, 'co_captain'::STRING, 'member'::STRING, 'observer'::STRING)))`),]);
...
export const newTeamMaterials = cockroachTable("new_team_materials", {
	...
	index("idx_new_team_materials_category").using("btree", table.category.asc()),
...
export const newTeamMemberships = cockroachTable("new_team_memberships", {
	...
	index("idx_new_team_memberships_archived").using("btree", table.status.asc()).where(sql`status = 'archived'::STRING`),
	index("idx_new_team_memberships_deleted").using("btree", table.status.asc()).where(sql`status = 'deleted'::STRING`),
	...
	index("idx_new_team_memberships_team_id").using("btree", table.teamId.asc()),
	index("idx_new_team_memberships_user_id").using("btree", table.userId.asc()),
	uniqueIndex("new_team_memberships_user_id_team_id_key").using("btree", table.userId.asc(), table.teamId.asc()),
check("new_team_memberships_check_role", sql`CHECK (("role" IN ('captain'::STRING, 'co_captain'::STRING, 'member'::STRING, 'observer'::STRING)))`),check("new_team_memberships_status_check", sql`CHECK ((status IN ('active'::STRING, 'inactive'::STRING, 'pending'::STRING, 'banned'::STRING, 'archived'::STRING)))`),]);
...
export const newTeamNotifications = cockroachTable("new_team_notifications", {
	...
	index("idx_new_team_notifications_is_read").using("btree", table.isRead.asc()),
	index("idx_new_team_notifications_user_id").using("btree", table.userId.asc()),
]);
...
export const newTeamPollVotes = cockroachTable("new_team_poll_votes", {
	...
	uniqueIndex("new_team_poll_votes_poll_id_user_id_key").using("btree", table.pollId.asc(), table.userId.asc()),
]);
...
export const newTeamPosts = cockroachTable("new_team_posts", {
	...
	index("idx_new_team_posts_created_at").using("btree", table.createdAt.desc()),
	index("idx_new_team_posts_team_id").using("btree", table.teamId.asc()),
check("check_post_type", sql`CHECK ((post_type IN ('announcement'::STRING, 'assignment'::STRING, 'material'::STRING, 'event'::STRING)))`),check("check_priority", sql`CHECK ((priority IN ('low'::STRING, 'normal'::STRING, 'high'::STRING, 'urgent'::STRING)))`),]);
...
export const newTeamRecurringMeetings = cockroachTable.withRLS("new_team_recurring_meetings", {
	...
	index("idx_recurring_meetings_created_by").using("btree", table.createdBy.asc()),
	index("idx_recurring_meetings_end_date").using("btree", table.endDate.asc()),
	index("idx_recurring_meetings_start_date").using("btree", table.startDate.asc()),
...
export const newTeamRemovedEvents = cockroachTable("new_team_removed_events", {
	...
	removedAt: timestamp("removed_at", { mode: 'string', withTimezone: true }).defaultNow(),
}, (table) => [
	index("idx_removed_events_conflict_block").using("btree", table.conflictBlock.asc()),
	index("idx_removed_events_team_conflict").using("btree", table.teamUnitId.asc(), table.conflictBlock.asc()),
	index("idx_removed_events_team_unit").using("btree", table.teamUnitId.asc()),
	uniqueIndex("new_team_removed_events_team_unit_id_event_name_key").using("btree", table.teamUnitId.asc(), table.eventName.asc()),
]);
...
export const newTeamRosterData = cockroachTable("new_team_roster_data", {
	...
	index("idx_roster_data_lookup").using("btree", table.teamUnitId.asc(), table.eventName.asc(), table.slotIndex.asc()),
	index("idx_roster_data_team_event").using("btree", table.teamUnitId.asc(), table.eventName.asc()),
	index("idx_roster_data_team_unit").using("btree", table.teamUnitId.asc()),
	index("idx_roster_data_team_user").using("btree", table.teamUnitId.asc(), table.userId.asc()),
	index("idx_roster_data_user_id").using("btree", table.userId.asc()),
	uniqueIndex("new_team_roster_data_team_unit_id_event_name_slot_index_key").using("btree", table.teamUnitId.asc(), table.eventName.asc(), table.slotIndex.asc()),
]);
...
export const newTeamStreamComments = cockroachTable("new_team_stream_comments", {
	...
	index("idx_stream_comments_created_at").using("btree", table.createdAt.desc()),
...
export const newTeamStreamPosts = cockroachTable("new_team_stream_posts", {
	...
	index("idx_stream_posts_team_created").using("btree", table.teamUnitId.asc(), table.createdAt.desc()),
...
export const newTeamUnits = cockroachTable("new_team_units", {
	...
	index("idx_new_team_units_archived").using("btree", table.status.asc()).where(sql`status = 'archived'::STRING`),
	index("idx_new_team_units_codes").using("btree", table.captainCode.asc(), table.userCode.asc()),
	index("idx_new_team_units_deleted").using("btree", table.status.asc()).where(sql`status = 'deleted'::STRING`),
	index("idx_new_team_units_group_display_order").using("btree", table.groupId.asc(), table.displayOrder.asc()),
	...
	uniqueIndex("new_team_units_captain_code_key").using("btree", table.captainCode.asc()),
	uniqueIndex("new_team_units_group_id_team_id_key").using("btree", table.groupId.asc(), table.teamId.asc()),
	uniqueIndex("new_team_units_user_code_key").using("btree", table.userCode.asc()),
check("new_team_units_status_check", sql`CHECK ((status IN ('active'::STRING, 'archived'::STRING, 'deleted'::STRING)))`),]);

export const notifications = cockroachTable("notifications", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	userId: uuid("user_id").notNull(),
	type: string().notNull(),
	...
	index("idx_notifications_user").using("btree", table.userId.asc(), table.isRead.asc()),
]);

export const questions = cockroachTable("questions", {
	id: uuid().primaryKey(),
	question: string().notNull(),
	...
	index("idx_questions_event_division").using("btree", table.event.asc(), table.division.asc()),
	index("idx_questions_event_division_difficulty").using("btree", table.event.asc(), table.division.asc(), table.difficulty.asc()),
	...
	index("questions_division_random_f_idx").using("btree", table.division.asc(), table.randomF.asc()),
	index("questions_event_random_f_idx").using("btree", table.event.asc(), table.randomF.asc()),
	index("questions_question_event_idx").using("btree", table.question.asc(), table.event.asc()),
	index("questions_random_f_idx").using("btree", table.randomF.asc()),
]);

export const quotes = cockroachTable("quotes", {
	id: uuid().defaultRandom().primaryKey(),
	...
	index("idx_quotes_language_char_length").using("btree", table.language.asc(), table.charLength.asc()),
	index("idx_quotes_language_id").using("btree", table.language.asc(), table.id.asc()),
	index("quotes_language_random_f_idx").using("btree", table.language.asc(), table.randomF.asc()),
...
export const rosterLinkInvitations = cockroachTable("roster_link_invitations", {
	...
	index("idx_roster_link_invitations_team_student").using("btree", table.teamId.asc(), table.studentName.asc()),
	index("idx_roster_link_invitations_user").using("btree", table.invitedUserId.asc()),
	uniqueIndex("roster_link_invitations_team_id_student_name_invited_user_id_key").using("btree", table.teamId.asc(), table.studentName.asc(), table.invitedUserId.asc()),
check("roster_link_invitations_check_status", sql`CHECK ((status IN ('pending'::STRING, 'accepted'::STRING, 'declined'::STRING, 'expired'::STRING)))`),]);

export const shareLinks = cockroachTable("share_links", {
	id: uuid().defaultRandom().primaryKey(),
	code: string().notNull(),
	indices: jsonb(),
...
export const teamGroupTournaments = cockroachTable("team_group_tournaments", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	...
	index("team_group_tournaments_group_idx").using("btree", table.groupId.asc()),
]);

export const teamGroups = cockroachTable("team_groups", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	school: varchar({ length: 255 }).notNull(),
	division: char().notNull(),
...
check("team_groups_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);

export const teamLinks = cockroachTable("team_links", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	school: varchar({ length: 255 }).notNull(),
	division: char().notNull(),
	teamId: varchar("team_id", { length: 10 }).notNull(),
	...
	index("idx_team_links_team").using("btree", table.school.asc(), table.division.asc(), table.teamId.asc()),
check("team_links_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);
...
export const teamMemberships = cockroachTable("team_memberships", {
	...
	uniqueIndex("team_memberships_user_id_team_unit_id_key").using("btree", table.userId.asc(), table.teamUnitId.asc()),
check("check_role", sql`CHECK (("role" IN ('captain'::STRING, 'user'::STRING)))`),]);

export const teamUnits = cockroachTable("team_units", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	school: varchar({ length: 255 }).notNull(),
	division: char().notNull(),
	...
	index("idx_team_units_codes").using("btree", table.captainCode.asc(), table.userCode.asc()),
	index("idx_team_units_school_division").using("btree", table.school.asc(), table.division.asc()),
	uniqueIndex("team_units_captain_code_key").using("btree", table.captainCode.asc()),
	uniqueIndex("team_units_slug_key").using("btree", table.slug.asc()),
	uniqueIndex("team_units_user_code_key").using("btree", table.userCode.asc()),
	uniqueIndex("uq_team_units_group_teamid").using("btree", table.groupId.asc(), table.teamId.asc()),
	uniqueIndex("uq_team_units_slug").using("btree", table.slug.asc()),
check("team_units_check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);

export const teams = cockroachTable("teams", {
	id: int8({ mode: 'number' }).default(sql`unique_rowid()`).primaryKey(),
	school: varchar({ length: 255 }).notNull(),
	division: char().notNull(),
	...
	index("idx_teams_school_division").using("btree", table.school.asc(), table.division.asc()),
	index("idx_teams_user_code").using("btree", table.userCode.asc()),
	uniqueIndex("teams_captain_code_key").using("btree", table.captainCode.asc()),
	uniqueIndex("teams_school_division_unique").using("btree", table.school.asc(), table.division.asc()),
	uniqueIndex("teams_user_code_key").using("btree", table.userCode.asc()),
check("check_division", sql`CHECK ((division IN ('B'::STRING, 'C'::STRING)))`),]);
...
export const teamsMembership = cockroachTable("teams_membership", {
	...
	uniqueIndex("teams_membership_unique").using("btree", table.teamId.asc(), table.userId.asc()),
]);
...
export const teamsRoster = cockroachTable("teams_roster", {
	...
	uniqueIndex("teams_roster_unique").using("btree", table.teamId.asc(), table.subteamId.asc(), table.eventName.asc(), table.slotIndex.asc()),
]);
...
export const teamsSubmission = cockroachTable("teams_submission", {
	...
	index("idx_teams_submission_assignment_id").using("btree", table.assignmentId.asc()),
	uniqueIndex("teams_submission_unique").using("btree", table.assignmentId.asc(), table.userId.asc()),
]);
...
export const teamsSubteam = cockroachTable("teams_subteam", {
	...
	uniqueIndex("teams_subteam_name_unique").using("btree", table.teamId.asc(), sql`true`),
]);

export const teamsTeam = cockroachTable("teams_team", {
	id: uuid().defaultRandom().primaryKey(),
	slug: string().notNull(),
	name: string().notNull(),
	...
	createdAt: timestamp("created_at", { mode: 'string', withTimezone: true }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string', withTimezone: true }).defaultNow().notNull(),
	memberCode: string("member_code").default("substr(replace(gen_random_uuid()::STRING, '-'::STRING, ''::STRING), 1, 8)").notNull(),
	captainCode: string("captain_code").default("substr(replace(gen_random_uuid()::STRING, '-'::STRING, ''::STRING), 1, 10)").notNull(),
}, (table) => [
	uniqueIndex("teams_team_captain_code_unique").using("btree", table.captainCode.asc()),
	uniqueIndex("teams_team_member_code_unique").using("btree", table.memberCode.asc()),
...
export const users = cockroachTable("users", {
	...
	uniqueIndex("users_supabase_user_id_unique").using("btree", table.supabaseUserId.asc()),
	uniqueIndex("users_supabase_username_unique").using("btree", table.supabaseUsername.asc()),
	uniqueIndex("users_username_key").using("btree", table.username.asc()),
]);
export const newTeamMemberDetails = cockroachView("new_team_member_details", {	id: uuid(),
	userId: uuid("user_id"),
	teamId: uuid("team_id"),
	role: varchar({ length: 20 }),
	status: varchar({ length: 20 }),
	joinedAt: customType({ dataType: () => 'timestamp with time zone' })("joined_at"),
	invitedBy: uuid("invited_by"),
